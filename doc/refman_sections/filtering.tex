...
To discard multiple diagrams based on the internal propagators, the one or more of the options \texttt{filter[|.lo|.nlo|.ct].particles} can be used. These options discard diagrams which do not have the specified number of internal propagators of the given field. These options are applied at the diagram generation level and are therefore more efficient compared to an equivalent \python filter.
