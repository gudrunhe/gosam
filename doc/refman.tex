...
\item[50] \texttt{abbrev.limit}: maximum number of instructions per subroutine when calculating  
 abbreviations. The default is 0, which means that no maximum is set.
\item[51] \texttt{templates}:   Path pointing to the directory containing the template 
...
\item[52] \texttt{qgraf.bin}: path to the {\tt QGraf} executable.
An automatic search is attempted if left empty.
\item[53] \texttt{form.bin}:  path to the \form{} executable.
An automatic search is attempted if left empty.
\item[54] \texttt{form.threads}: the number of \form{} threads when using {\tt tform},
the parallel version of \form.  The default is 2.
\item[55] \texttt{form.tempdir}: the temporary directory where \form{}
can store (large) intermediate files.  the default is {\tt /tmp}.
\item[56] \texttt{haggies.bin}: path to the {\tt haggies} executable.
An automatic search is attempted if left empty.
\item[57] \texttt{fc.bin}: path to the {\tt Fortran} compiler.
An automatic search is attempted if left empty.
%{\tt /usr/bin/gfortran}.
\item[58] \texttt{python.bin}:  path to the {\tt python} executable.
An automatic search is attempted if left empty.
\item[59] \texttt{ninja.fcflags}: compiler flags to compile with \ninja.
An automatic search is attempted if left empty.
\item[60] \texttt{ninja.ldflags}: LDFLAGS required to link the \ninja{} library.
An automatic search is attempted if left empty.
\item[61] \texttt{samurai.fcflags}:  compiler flags to compile with \samurai.
An automatic search is attempted if left empty.
\item[62] \texttt{samurai.ldflags}: LDFLAGS required to link the \samurai{} library.
An automatic search is attempted if left empty. 
\item[63] \texttt{golem95.fcflags}: compiler flags to compile with \golemVC{}.
An automatic search is attempted if left empty. 
\item[64] \texttt{golem95.ldflags}:  LDFLAGS required to link the \golemVC{} library.
An automatic search is attempted if left empty. 
\item[65] \texttt{r2}: treatment of the rational part $R_2$. The possibilities are
\begin{itemize}
