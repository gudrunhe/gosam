...
\begin{itemize}
   ...
   \item The model's $d=6$ SMEFT couplings specify \texttt{NP=1} in their \texttt{order} attribute. Note that \texttt{NP} is the conventional name for BSM/new physics orders in the UFO format. Some SMEFT models might assign \texttt{NP=2} to the $d=6$ terms instead. In this case the user should adjust the model accordingly.
...
%
% \begin{align}
%    \left|\mathcal{M}_i\right|^2 &= \underbrace{\underbrace{\underbrace{\left|\mathcal{M}_i^{\mathrm{SM}}\right|^2}_{\texttt{EFTcount=0}} + \overbrace{\frac{2\,\mathrm{Re}\left\{\mathcal{M}_i^{\mathrm{SM}\,\dagger}\mathcal{M}_i^{6}\right\}}{\Lambda^2}}^{\texttt{EFTcount=3}}}_{\texttt{EFTcount=1}} + \overbrace{\frac{\left|\mathcal{M}_i^{6}\right|^2}{\Lambda^4}}^{\texttt{EFTcount=4}}}_{\texttt{EFTcount=2}}
% \end{align}
%
% \begin{multline}
%    2\,\mathrm{Re}\left\{\mathcal{M}_\mathrm{tree}^\dagger\,\mathcal{M}_\mathrm{1-loop}\right\} \\[5pt]
%       = 2\,\mathrm{Re}\left\{\mathcal{M}_\mathrm{tree}^{\mathrm{SM}\,\dagger}\,\mathcal{M}_\mathrm{1-loop}^{\mathrm{SM}}\right\}\hspace{100pt}\\[5pt]
%       + \frac{2\,\mathrm{Re}\left\{\mathcal{M}_\mathrm{tree}^{\mathrm{SM}\,\dagger}\,\mathcal{M}_\mathrm{1-loop}^{6} + \mathcal{M}_\mathrm{tree}^{6\,\dagger}\,\mathcal{M}_\mathrm{1-loop}^{\mathrm{SM}}\right\}}{\Lambda^2}\\[5pt]
%       + \frac{2\,\mathrm{Re}\left\{\mathcal{M}_\mathrm{tree}^{6\,\dagger}\,\mathcal{M}_\mathrm{1-loop}^{6}\right\}}{\Lambda^4}
% \end{multline}
