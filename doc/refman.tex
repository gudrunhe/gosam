...
\end{align}
where $d$ denotes the canonical dimension of the operators $O^d_{i_d}$, with corresponding Wilson coefficients $C^d_{i_d}$. In the following we will neglect $d=5$ terms\footnote{There are only two lepton-number-violating operators. Experimental findings suggest them to be extremely suppressed.} and truncate the SMEFT expansion at $d=6$ (dim-6). Any amplitude derived from the Lagrangian~(\ref{eq:SMEFTLag}) can then be written as
\begin{align}
   \mathcal{M}_i &= \mathcal{M}_i^{\mathrm{SM}} + \frac{\mathcal{M}_i^{6}}{\Lambda^2} \qquad i=\text{tree}\,,\text{1-loop}\,.
\end{align}
Physical quantities depend on the square of such amplitudes, which immediately raises the question of how to truncate such squares. Schematically for tree- and loop-induced processes,
\begin{align}
   \left|\mathcal{M}_i\right|^2 &= \left|\mathcal{M}_i^{\mathrm{SM}}\right|^2 + \frac{2\,\mathrm{Re}\left\{\mathcal{M}_i^{\mathrm{SM}\,\dagger}\mathcal{M}_i^{6}\right\}}{\Lambda^2} + \frac{\left|\mathcal{M}_i^{6}\right|^2}{\Lambda^4}\,.
\end{align}
For genuine 1-loop corrections it is understood that the square is actually an interference with the Born amplitude. The last term in above expression is technically of the same order in $\Lambda$ as an interference between a dim-4 (SM) and a dim-8 contribution. The latter is not included in the model (and won't be handled correctly by \gosam when doing so), so consequently the $\text{dim-6}^2$ term should be dropped in a consistent treatment. On the other hand however, only taking the $\Lambda^{-2}$ (linear) interference term into account can lead to negative cross-sections for certain pathological points in the EFT parameter space, which is purely an artifact of the truncation. For this reason it is sometimes desirable to keep the $\text{dim-6}^2$ (quadratic) term.

\gosam enables to user to choose between different truncation options by setting \texttt{enable\_truncation\_orders=true} in the process card, provided the process setup and model used meet some requirements:
\begin{itemize}
   \item The model is provided in the UFO format.
   \item The model's dim-6 SMEFT couplings specify \texttt{NP=1} in their \texttt{order} attribute. Note that \texttt{NP} is the conventional name for BSM/new physics orders in the UFO format. Some SMEFT models might assign \texttt{NP=2} to the dim-6 terms instead. In this case the user should adjust the model accordingly.
...
\end{itemize}
The desired truncation order can be selected by means of the variable \texttt{EFTcount}. Possible values are shown in table~\ref{tab:EFTcount}

\begin{table}
\renewcommand{\arraystretch}{1.5}
\begin{tabular}{c|l|l}
   \texttt{EFTcount} & truncation & \\
\hline
   0 & $\text{SM}^2$ & pure SM\\
   1 & $\text{SM}^2 + \text{SM}\otimes\text{dim-6}$ & linear truncation\\
   2 & $\left(\text{SM}+\text{dim-6}\right)^2$ & quadratic truncation\\
   3 & $\text{SM}\otimes\text{dim-6}$ & linear coefficient\\
   4 & $\text{dim-6}^2$ & quadratic coefficient
\end{tabular}
...
A special remark has to be made about double, or in general multiple, insertions of dim-6 operators. Per default \gosam will generate diagrams with multiple insertions of non-SM vertices, if they are present in the model. However, in a SMEFT context this leads to inconsistencies as there is an intricate interplay between diagrams with multiple insertions of dim-6 operators and those diagrams with higher dimensional ($d>6$) operators which are technically of the same order. To avoid such problems and to set up a consistent SMEFT calculation the user should use the \python diagram filters to single out diagrams with at most one SMEFT vertex:
\begin{lstlisting}[gobble=3,%
     basicstyle=\ttfamily]
...
\textcolor{red}{MH: Say something about loop counting? EFTcount options 11 through 14}

\gosam enables the user to also consider scenarios with loop-suppressed operators. The tree and 1-loop amplitudes can then be written as
\begin{align}
   \mathcal{M}_\text{tree} &= \mathcal{M}_\text{tree}^{\mathrm{SM}} + \frac{\mathcal{M}_\text{tree}^{6,0}}{\Lambda^2}\\
%
   \mathcal{M}_\text{1-loop} &= \mathcal{M}_\text{1-loop}^{\mathrm{SM}} + \frac{\mathcal{M}_\text{1-loop}^{6,0}}{\Lambda^2} + \frac{\mathcal{M}_\text{tree}^{6,1}}{\Lambda^2}\,.
\end{align}
Here $\mathcal{M}_\text{tree}^{6,0}$ and $\mathcal{M}_\text{1-loop}^{6,0}$ denote a tree level and 1-loop contribution, respectively,  with a single insertion of non-loop-suppressed EFT operator. They correspond to \texttt{NP=1} and \texttt{QL=0}. $\mathcal{M}_\text{tree}^{6,1}$ is the contribution of tree level topologies with a single insertion of a loop-suppressed operator, rendering the whole contribution effectively of 1-loop order. They correspond to \texttt{NP=1} and \texttt{QL=1}.
...
\end{multline}
where $1_\mathrm{DRED}=1$, if the calculation is performed in DRED (\gosam's default) and $1_\mathrm{DRED}=0$ in the t'Hoof-Veltman scheme. $\mu_\mathrm{EFT}$ is the scale at which the Wilson coefficients are renormalised, which can in general be different from the renormalisation scale $\mu_R$ of the strong coupling. The corresponding counterterm vertex is defined in \texttt{CT\_vertices.py}:
\begin{lstlisting}[gobble=3,%
     basicstyle=\ttfamily]
   CTV_1 = CTVertex(
      name = 'CTV_1',
...
The model then needs three additional files, \texttt{CT\_parameters.py}. \texttt{CT\_vertices.py} and \texttt{CT\_couplings.py}. The former two contain objects of the special UFO \python classes \texttt{CTparameter} and \texttt{CTvertex}, respectively.
