...
or \ninja~\cite{Mastrolia:2012bu,vanDeurzen:2013saa}.
...
(by {\tt source [path]/gosam\_setup\_env.sh}).
...
All  files which have been installed are tracked in the {\tt installer-log.ini} file.
...
All files are downloaded to the GoSam subfolder and extracted.
The build files remain on the system. One can delete the GoSam subfolder after
the installation. However, as they contain examples (./GoSam/gosam/examples)
...
any option. It determines the install location from the {\tt installer-log.ini} file
which is searched for in the current directory. The script will search 
...
a {\tt fortran} compiler and the chosen reduction libraries need to be present.
...
has not been run (successfully) before.
\end{enumerate}
...
\form is also distributed with the \gosam-2.0 package for the user's convenience.
...
is also included in the \gosamv{} distribution already.
...
The libraries are already distributed with the code. Alternatively, 
\begin{itemize}
...
for all defined options. The options are filled with some default values,
which can be set in a global configuration file.
The script will search\footnote{in this order} in the \gosamv{} directory,
...
11    true=iprop[Z, 0, 0];\n\
12    true=iprop[H, 0, 0];
13 extensions=samurai,numpolvec
14 samurai.fcflags=path_to_gosam-contrib/include_files
15 samurai.ldflags=path_to_gosam-contrib/libraries
\end{lstlisting}
...
10--12 suppress the generation of diagrams containing Higgs and $Z$ bosons.
...
\\These options can be omitted.
\item[13] The option \texttt{extensions} contains a list of extensions
to the core of the program. It should contain at least one of the reduction libraries 
...
directory of \gosamv. The original files are renamed, e.g.
