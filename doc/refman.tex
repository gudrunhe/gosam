...
\item[36] \texttt{nlo\_prefactors}:  can take the integer values 0,1,2, which have the        
    ...
    \item[2]: the NLO result includes all prefactors   (see also section \ref{conventions}).
    \end{enumerate}                           
...
4.x~\cite{Kuipers:2012rf} to produce optimized code. This lead to more
...
i.e. if {\tt noformopt=True} is used, please note the  following:
...
when reducing using an integrand reduction method, as well as with
...
still present when \samurai{} or \golemVC{} are used for computing the
amplitudes.
...
In the new release an option called {\tt diagsum} combines diagrams
...
the option {\tt polvec=explicit} should be given in the input file 
{\tt process.in}.
...
{\tt extension} options in \gosam-1.0 already, it now has been promoted to 
...
The number of optional indices $i_1$, $i_2$, \dots presented to the routine
...
A first commonly used approach relies on the comparison between the numerical values of the pole coefficients computed by the automated tool with their known analytic results dictated by the universal behaviour of the infrared singularities~\cite{Catani:2000ef}. We refer to this as the {\it pole test}. 
...
code itself fully support complex masses. This refers to the introduction of fermion widthes
but in particular also for $W$- and $Z$-bosons. A fully consistent treatment of complex
...
\begin{equation}
 \cos\theta_W^2 = \frac{\mu_W^2}{\mu_Z^2}\;.
\end{equation}
 ...
 with a full CKM matrix, the latter with a diagonal unit matrix for the CKM matrix.
 An example dealing with a complex top quark mass is given in section \ref{sec:examples}.
