...
Using \gosam requires a Python installation, a Fortran compiler, a C++ compiler, \textsc{Make}, and the \textsc{Meson} build system. It is tested with Python $\geq 3.9$ and \textsc{GNU~Fortran}.
...
\marginlabel{\python (1)} \gosam requires Python version 3{.}9 or newer.
...
\marginlabel{\tt common/} Contains Fortran files which are common to all helicity
...
compact code and a speed-up in amplitude evaluation.
...
\section{The extension {\tt derive} ... is not an extension anymore}
\label{sec:derive}
...
\texttt{examples/model/}, as well as UFO files for ADD~\cite{ArkaniHamed:1998rs}
