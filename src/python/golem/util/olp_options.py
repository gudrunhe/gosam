...
def Model(values, conf, ignore_case):
    ...
    return expect_one_keyword(values, conf, ignore_case, "model", supported_values)
...
def WidthScheme(values, conf, ignore_case):
    ...
    ret = expect_one_keyword(values, conf, True, "olp.widthscheme", supported_values)
    if ret == "ComplexMass":
