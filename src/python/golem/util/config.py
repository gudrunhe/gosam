...
class Ninja(Library):
    ...
    def examine(self, hints):
        ...
        if len(self.locations) > 0:
            self.incdirs = self.findIncludeDir("gosam-contrib", "ninjago_module", hints, ".mod") or self.findIncludeDir(
                "ninja-contrib", "ninjago_module", hints, ".mod"
            )
...
class Golem95(Library):
    ...
    def examine(self, hints):
        Library.examine(self, hints)
        if len(self.locations) > 0:
            self.incdirs = self.findIncludeDir("gosam-contrib", "parametre", hints, ".mod") or self.findIncludeDir(
                "golem95", "parametre", hints, ".mod"
            )
            if len(self.incdirs) == 0:
                self.locations = []
    ...
    def examine(self, hints):
        ...
        try:
            pipe = subprocess.Popen(
                executable, shell=True, bufsize=500, stdout=subprocess.PIPE, encoding="utf-8"
            ).stdout
            ...
            for line in pipe.readlines():
                ...
                if "version" in lline:
                    ...
                    self.version = [int(re.sub("[^0-9]", "", s)) for s in lline.split(".")]
            ...
            raise ConfigurationException("Could not run FORM (%s) properly" % executable)
    ...
    def store(self, conf):
        ...
        try:
            if "GNU" in subprocess.run([fc, "--version"], capture_output=True, text=True).stdout:
                gfortran_version = int(
                    subprocess.run([fc, "-dumpversion"], capture_output=True, text=True).stdout.split(".")[0]
                )
        except:
            ...
            for loc in self.locations:
                ...
                if "mold" in exe:
                    if gfortran_version >= 12:
                        ...
                        logger.info(
                            "Found linker 'mold', but the used version of gfortran is not compatible. Using default linker."
                        )
                    break
                elif "lld" in exe:
                    if gfortran_version >= 9:
                        ...
                        logger.info(
                            "Found linker 'lld', but the used version of gfortran is not compatible. Using default linker."
                        )
                    break
                elif "gold" in exe:
                    if gfortran_version >= 5:
                        ...
                        logger.info(
                            "Found linker 'gold', but the used version of gfortran is not compatible. Using default linker."
                        )
                    break
    ...
    def __init__(self, hints, **components):
        ...
        for name, required in items:
            ...
            if type(cls) != type(Component):
                raise ConfigurationException("Name '%s' does not denote a class" % name)
            if not issubclass(cls, Component):
                raise ConfigurationException("Class '%s' is not a subclass of 'Component'" % name)
...
def testCompilerLibCompatibility(compiler, lib, flags):
    ...
    try:
        ...
        p = subprocess.Popen(
            [compiler, "program.f90", "-c"] + shlex.split(flags),
