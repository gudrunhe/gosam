...
def generate_process_files(conf, from_scratch=False):
    ...
    if lo_diagrams is not None:
        ...
        props.setProperty(
            "topolopy.keep.tree", ",".join(map(str, range(1, len(lo_diagrams) + 1)))
        )
        props.setProperty("topolopy.count.tree", len(lo_diagrams))
        conf.setProperty("topolopy.count.tree", len(lo_diagrams))
    else:
        props.setProperty("topolopy.keep.tree", "")
        conf.setProperty("topolopy.count.tree", 0)
    ...
    if ct_diagrams is not None:
        props.setProperty(
            "topolopy.keep.ct", ",".join(map(str, range(1, len(ct_diagrams) + 1)))
        )
        props.setProperty("topolopy.count.ct", len(ct_diagrams))
    else:
        props.setProperty("topolopy.keep.ct", "")
        props.setProperty("topolopy.count.ct", 0)
    ...
    if (
        conf.getBooleanProperty("generate_eft_loopind")
        and lo_diagrams is not None
    ...
    golem.templates.xmltemplates.transform_templates(
        ...
        lo_flags=flags[0],
        nlo_flags=flags[1],
        diagram_sum=nlo_props["eprops"] if nlo_props is not None else {},
...
def read_golem_dir_file(path):
    ...
    if os.path.exists(dir_file):
        ...
        if (
            ver[:2] == [2, 0]
            and GOLEM_VERSION[:2] == [2, 0]
            and ver[:3] <= (GOLEM_VERSION[:3] + [0] * 5)[:3]
        ):
...
def write_golem_dir_file(path, fname, conf):
    ...
    if os.path.exists(os.path.abspath(fname)):
        dir_info["setup-file-sha1"] = hashlib.sha1(
...
def fill_config(conf):
    ...
    experimentals = list(
        map(str, [p for p in golem.properties.properties if p.isExperimental()])
    )
    for name in conf:
    ...
    if not conf.getBooleanProperty("__OLP_MODE__"):
        ...
        if (
            True
            if conf.getProperty("order") is None
            else conf.getProperty("order") == ""
        ):
            raise GolemConfigError(
    ...
    if all(len(p) == 2 for p in orders):
        ...
        generate_tree_diagrams = not any(
            str(p[1]).strip().lower() == "none" for p in orders
        )
        generate_loop_diagrams = True
        is_loopinduced = not generate_tree_diagrams
    else:
        raise GolemConfigError(
            "The property %s must have 2 or 3 arguments."
            % golem.properties.coupling_power
        )
    ...
    if raise_warn:
        logger.warning(
            "You explicitly turned off QCD renormalisation by setting renorm=False.\n"
    ...
    if not (conf["is_ufo"] or ("FeynRules" in conf.getProperty("model"))):
        ...
        if conf.getBooleanProperty("enable_truncation_orders"):
            raise_err = True
            err_str = (
                err_str + ", enable_truncation_orders"
                if err_str
                else "enable_truncation_orders"
            )
            err_count += 1
        ...
        if conf.getBooleanProperty("use_vertex_labels"):
            raise_err = True
            err_str = (
                err_str + ", use_vertex_labels" if err_str else "use_vertex_labels"
            )
            err_count += 1
        if conf.getBooleanProperty("loop_suppressed_Born"):
            raise_err = True
            err_str = (
                err_str + ", loop_suppressed_Born"
                if err_str
                else "loop_suppressed_Born"
            )
            err_count += 1
        if raise_err:
            if err_count > 1:
    ...
    if conf.getBooleanProperty("renorm_eftwilson") and conf.getBooleanProperty(
        "renorm_ehc"
    ):
        raise GolemConfigError(
    ...
    if not golem.installation.WITH_GOLEM:
        if reduction_programs == ["golem95"]:
            raise GolemConfigError(
                "The configuration requests only 'golem95' as reduction library,"
            ...
            logger.warning(
                "The configuration requests 'golem95' as a reduction library, but GoSam was build without Golem.\n"
    ...
    if not conf.getBooleanProperty("__OLP_MODE__"):
        ...
        if conf["regularisation_scheme"] == "dred":
            ...
            logger.critical(
                "Unknown regularisation_scheme in config: %s"
                % str(conf["regularisation_scheme"])
            )
            sys.exit("GoSam terminated due to an error")
