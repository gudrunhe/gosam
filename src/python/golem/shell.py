...
class GolemShell(golem.util.ishell.InteractiveShell):
    def __init__(self, prompt="GoSam> "):
        self.golem_follow_set = {
            ...
            str(golem.properties.model): ["sm", "smdiag", "FeynRules", " file"],
            ...
            str(golem.properties.abbrev_level): ["diagram", "group", "helicity"],
            str(golem.properties.r2): ["implicit", "explicit", "only", "off"],
            str(golem.properties.symmetries): ["flavour", "family", "lepton", "generation", "parity"],
        ...
        golem.util.ishell.InteractiveShell.__init__(
            self,
            prompt=prompt,
    ...
    def banner(self):
        ...
        print(CMTCH * WIDTH)
        for line in golem.util.tools.banner(WIDTH=WIDTH, PREFIX=CMTCH, SUFFIX=CMTCH):
            print(line)
        print(CMTCH * WIDTH)
    ...
    def update_model(self):
        ...
        try:
            ...
            exec(compile(python_file.getvalue(), "model.py", "exec"), mdl_dict, mdl_dict)
            self.model = mdl_dict
        ...
        self.golem_follow_set[str(golem.properties.model_options)] = ["%s:" % name for name in parameter_names]
    ...
    def event(self, line):
            ...
            if all([arg == " " for arg in args]):
                ...
                if sargs.startswith("!"):
                    ...
                    try:
                        ...
                        if retcode != 0:
                            print("Child process terminated with exit code %d." % retcode)
                    except OSError as e:
        ...
        for cmd in COMMANDS:
            if str(cmd) == keyword:
                ...
                self.golem_follow_set["accept"] = list(self.recommendations.keys())
...
class Command:
    def __init__(self, name, help_text=None, followset=[]):
        ...
        if help_text is None:
            self.help_text = ["No help available for command %r." % name]
        else:
...
class MAKE(Command):
    def __init__(self):
        Command.__init__(
            ...
            followset=["source", "compile", "doc", "dist", "clean", "very-clean"],
        )
...
class RESET(Command):
    def __init__(self):
        Command.__init__(self, "reset", ["Set all parameters to their defaults and start a new session."])
...
class ACCEPT(Command):
    def __init__(self):
        Command.__init__(self, "accept", ["Accept the recommended settings suggested by the previous" + " command."])
...
class SAVE(Command):
    def __init__(self):
        Command.__init__(
            self,
            "save",
            ["Saves the commands of this session into a file.", "See also: load, reset"],
            followset=[" file"],
        )
...
class LOAD(Command):
    def __init__(self):
        Command.__init__(
            self, "load", ["Loads a previously saved session.", "See also: save, reset"], followset=[" file"]
        )
...
class GENERATE(Command):
    ...
    def run(self, shell, args):
        ...
        if len(fname) == 0:
            ...
            process_path = conf.getProperty(golem.properties.process_path, "").strip()
            process_name = conf.getProperty(golem.properties.process_name, "").strip()

            orders = golem.util.config.split_qgrafPower(
                ",".join(map(str, conf.getListProperty(golem.properties.qgraf_power)))
            )
            ...
            if not conf["PSP_chk_method"] or conf["PSP_chk_method"].lower() == "automatic":
                conf["PSP_chk_method"] = (
                    "PoleRotation" if conf.getBooleanProperty("generate_lo_diagrams") else "LoopInduced"
                )
            ...
            if os.path.exists(ap):
                if not os.path.isdir(ap):
                    print("Your specified process path %r is not a directory." % process_path)
                    return True
            else:
            ...
            if os.path.exists(fname):
                if not os.path.isfile(fname):
                    ...
                    if "setup-file" in dir_info:
                        ...
                        print("The argument %r seems not to refer to a" % fname + "process file or directory.")
                        return True
                else:
                    in_file = fname
            else:
...
class HELP(Command):
    def __init__(self):
        ...
        Command.__init__(self, "help", ["Prints a help screen for the given topic."], followset=followset)
            ...
            for name, particle in list(particle_dict.items()):
                if help_topic == name:
                    ...
                    if field in shell.model["latex_names"]:
                        print("LaTeX:                %s" % shell.model["latex_names"][field])
...
COMMANDS = [QUIT(), MAKE(), RESET(), SAVE(), LOAD(), ACCEPT(), WRITE(), GENERATE(), LIST(), TOPDG()]
