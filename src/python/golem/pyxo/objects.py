...
class diagram:
    ...
    def layout(self, *args, **opts):
        ...
        if len(chords) == 0:
            ...
            loop_vertices = [self.propagators[list(chords)[0]].vertex1, self.propagators[list(chords)[0]].vertex2]
        else:
            loop_vertices = []
            while len(loop_vertices) < len(loop):
                for i, s in list(loop.items()):
                    ...
                    if s == 1:
                        p = [self.propagators[i].vertex1, self.propagators[i].vertex2]
                    else:
                        p = [self.propagators[i].vertex2, self.propagators[i].vertex1]
...
class propagator:
    ...
    def draw(self, f, diag, lookup=None, *args, **opts):
        ...
        f.write("   ")
        if self.bend == 0:
            self.field1.draw_line(f, v1.x, v1.y, v2.x, v2.y, lookup, label=label, **opts)
        elif self.bend == 1:
            mx = (v1.x + v2.x) / 2.0
            my = (v1.y + v2.y) / 2.0
            dx = v1.x - v2.x
            ...
            self.field1.draw_arc(f, mx, my, r, phi, phi + 180, lookup, label=label, **opts)
        elif self.bend == -1:
            mx = (v1.x + v2.x) / 2.0
            my = (v1.y + v2.y) / 2.0
            dx = v1.x - v2.x
            ...
            self.field1.draw_arcn(f, mx, my, r, phi, phi - 180, lookup, label=label, **opts)
        elif self.bend == 2:
            ...
            r = math.sqrt(dx * dx + dy * dy) / 2.0
            phi = math.degrees(math.atan2(dy, dx))
            self.field1.draw_arc(f, mx, my, r, phi - 180.0, phi + 180.0, lookup, label=label, **opts)
        else:
            logging.critical("don't know what to do: " + "self.bend = %r (%s)" % (self.bend, self.bend))
            sys.exit("GoSam terminated due to an error")
...
class leg:
    ...
    def draw(self, f, diag, lookup=None, *args, **opts):
        ...
        if s1 * s2 > 0:
            self.field.draw_line(f, self.x, self.y, v.x, v.y, lookup, label=label, labelpos="start", **opts)
        else:
            self.field.draw_line(f, v.x, v.y, self.x, self.y, lookup, label=label, labelpos="end", **opts)
...
class field:
    ...
    def linestyle(self, lookup=None):
        ...
        styles = {
            ...
            "ghost": ["\\DashArrowLine", "\\DashArrowArc", "\\DashArrowArcn", "ghdashsize"],
            "scalar": ["\\DashLine", "\\DashCArc", "\\DashCArc", "sdashsize"],
            "gluon": ["\\Gluon", "\\GlueArc", "\\GlueArc", "gamplitude", "windings"],
            "photon": ["\\Photon", "\\PhotonArc", "\\PhotonArc", "pamplitude", "wiggles"],
            "chargedscalar": ["\\DashArrowLine", "\\DashArrowArc", "\\DashArrowArcn", "sdashsize"],
            "invisible": ["", "", "", ""],
        }
        ...
        defaults = {
            0: {1: "scalar", 3: "scalar", 8: "scalar"},
            1: {1: "fermion", 3: "fermion", 8: "fermion"},
            2: {1: "photon", 3: "photon", 8: "gluon"},
            3: {1: "fermion", 3: "fermion", 8: "fermion"},
            4: {1: "photon", 3: "photon", 8: "gluon"},
        }
    ...
    def __repr__(self):
        return "field(%r, %r, %r, %r, %r)" % (self.name, self.mass, self.is_conjugate, self.two_spin, self.color)
