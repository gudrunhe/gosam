...
class TableReader:
    ...
    def load(self, f):
        ...
        if num_columns < 1:
            raise CalcHEPImportError("Could not find sufficient number of columns.")
    ...
    def load(self, path, idx):
        ...
        for stub in stubs:
            try:
                ...
                raise CalcHEPImportError("While reading %r: %s" % (fname, exc.value()))
...
""")
        ...
        for row in lgrng.table:
            ...
            if len(field) == 4:
                vtxname = "%s  %s  %s  %s  Vertex" % (row[0], row[1], row[2], row[3])
            else:
                vtxname = "%s  %s  %s  Vertex" % (row[0], row[1], row[2])
            # message(vtxname)
    ...
    def color_generator(self, reps, indices):
        ...
        if len(reps) == 0:
            ...
            if abs(reps[0]) != 8:
                ...
                return T(SpecialExpression(indices[0]), SpecialExpression(indices[1]))
...
def grep_clifford_in_product(expr, is_vff):
    ...
    if len(cliff_factors) > 0:
        new_factors.append((1, NCContainer(ProductExpression(cliff_factors), adx, idx)))
