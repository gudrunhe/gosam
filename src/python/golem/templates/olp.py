...
class OLPTemplate(golem.util.parser.Template):
    ...
    def subprocesses(self, *args, **opts):
        ...
        numin_name = self._setup_name("num_in", prefix + "num_in", opts)
        numlegs_name = self._setup_name("num_legs", prefix + "num_legs", opts)
        numhelis_name = self._setup_name("num_helicities", prefix + "num_helicities", opts)
        ...
        for index, subprocess in enumerate(self._subprocesses):
            ...
            props[numin_name] = len(subprocess.p_ini)
    ...
    def generated_helicities(self, *args, **opts):
        if len(self._pstack) == 0:
            raise TemplateError("[% @for crossings %] outside [% @for subprocesses %]")
    ...
    def crossings(self, *args, **opts):
        ...
        amplitudetype = self._setup_name("amplitudetype", prefix + "amplitudetype", opts)
        notreelevel = self._setup_name("notreelevel", prefix + "notreelevel", opts)
