...
from golem.util.tools import enumerate_helicities, encode_helicity, enumerate_and_reduce_helicities
...
class _TemplateState:
    ...
    def create_directory(self, dest):
        if os.path.exists(dest):
            if not os.path.isdir(dest):
                logger.critical("Cannot create directory %r " % dest + "because it exists and is not a directory.")
    ...
    def delete_dir_if_empty(self, dest):
        if not os.path.exists(dest):
            logger.critical("Cannot check if directory %r " % dest + "is empty because it does not exist.")
            sys.exit("GoSam terminated due to an error")

        elif not os.path.isdir(dest):
            logger.critical("Cannot check if %r " % dest + "is empty because it is not a directory.")
            sys.exit("GoSam terminated due to an error")
        else:
    ...
    def transform_template_file(self, in_file, out_file, class_name, filter, executable):
        if out_file in self.produced_files:
            logger.warning("File %r has been overwritten while processing %r" % (out_file, in_file))
        else:
            self.produced_files.append(out_file)

        self.factory.process(
            in_file, out_file, class_name, self.props, self.opts, self.opts, filter=filter, executable=executable
        )

    def start_template(self, attrs):
        for name in ["description", "version", "author", "author-email", "maintainer", "maintainer-email"]:
            if name in attrs:
        ...
        if "golem-version-min" in attrs:
            golem_version_min = list(map(int, attrs["golem-version-min"].split(".")))
            if compare_version(GOLEM_VERSION, golem_version_min) < 0:
                raise TemplateXMLError(
                    "GoSam too old: template requires version %s or above." % attrs["golem-version-min"]
                )

        if "golem-version-max" in attrs:
            golem_version_max = list(map(int, attrs["golem-version-max"].split(".")))
            if compare_version(GOLEM_VERSION, golem_version_max) > 0:
                raise TemplateXMLError(
                    "GoSam too recent: template requires version %s or below." % attrs["golem-version-max"]
                )
        ...
        if "arguments" in attrs:
            ...
            for key in arguments:
                if key in env:
                    ...
                    raise TemplateXMLError("Undefined name in 'arguments': %r" % key)

            try:
                dest = dest % tuple(values)
            except TypeError:
                raise TemplateXMLError("In <file> attribute 'arguments' does not match 'dest'")
    ...
    def expand_directory(self, env, attrs):
        ...
        if "src" not in attrs:
            raise TemplateXMLError("<directory> without 'src' attribute encountered.")
        ...
        if "arguments" in attrs:
            ...
            try:
                ...
                raise TemplateXMLError("In <directory> attribute 'arguments' does not match 'dest'")
    ...
    def evaluate_conditions(self, env, attrs):
        ...
        if "if-file" in attrs:
            if l != 1:
                raise TemplateXMLError("No other attributes allowed with 'if-file'")
            value = attrs["if-file"]
            ...
            if value == "exists":
                ...
                raise TemplateXMLError(
                    "Unknown value %r for attribute 'if-file'." % value + "Must be 'exists' or 'generated'."
                )
        ...
        if "if-extension" in attrs:
            ...
            if "required" in attrs:
                ...
                raise TemplateXMLError(
                    "Attribute 'required' is mandantory if more than one " + "extension is listed in 'if-extension'."
                )

            if l != rlen:
                raise TemplateXMLError("Unknown attributes encountered near 'if-extension'")
        ...
        if "if-option" in attrs:
            ...
            if "value" in attrs:
                ...
                raise TemplateXMLError("Either 'value' or 'list' required with attribute 'if-option'")

            if len(attrs) != 2:
                raise TemplateXMLError("Unknown attributes encountered near 'if-option'")
            ...
            if option_value is None:
                ...
                if "true" in lvalues:
                    return option_value.strip().lower() in true_values or option_value in values
                elif "false" in lvalues:
                    return option_value.strip().lower() not in true_values or option_value in values
                else:
        ...
        if "if-internal" in attrs:
            ...
            if "required" in attrs:
                ...
                raise TemplateXMLError(
                    "Attribute 'required' is mandantory if more than one " + "extension is listed in 'if-internal'."
                )

            if l != rlen:
                raise TemplateXMLError("Unknown attributes encountered near 'if-internal'")
    ...
    def start_filter(self, attrs):
        ...
        for env in envs:
            ...
            if "name" in attrs:
                ...
                new_filter = golem.templates.filter.FilterFactory(attrs["name"], old_filter, **opts)
    ...
    def start_foreach(self, attrs):
        if "iterator" in attrs:
            if "list" in attrs:
                raise TemplateXMLError("<foreach> cannot have both atributes, 'iterator' and 'list'")
            ...
            if itername == "helicity":
                ...
                if "conf" in self.opts:
                    for i, heli in enumerate(enumerate_helicities(self.opts["conf"])):
                        gi, mapping, color_basis, permutation, gauge_set = mappings[i]
                        if i == gi:
                            values.append({"helicity": i, "helicitysymbol": encode_helicity(heli)})
                    ...
                    for i, crossing in enumerate(crossings):
                        if ":" in crossing:
                            ...
                            if ">" in process:
                                ...
                                values.append({"index": i, "name": name, "initial": ini, "final": fin})
                self.shuffle_push(values)
            else:
                raise TemplateXMLError("Unknown iterator at <foreach iterator=%r>" % itername)
        elif "list" in attrs:
            if "iterator" in attrs:
                raise TemplateXMLError("<foreach> cannot have both attributes, 'iterator' and 'list'")
            ...
            raise TemplateXMLError("<foreach> must have one of the attributes 'iterator' or 'list'")
...
def create_parser(template_dir, output_dir, *props, **opts):
    ...
    def evt_start_element(name, attrs):
        if name == "template":
            ...
            raise TemplateXMLError("Unknown element <%s> in xml file." % name)
