...
from golem.model.feynrules import cmath_functions, shortcut_functions, unprefixed_symbols, sym_cmath, sym_cmplx, i_
        ...
        for name, value in list(self._mod.slha_locations.items()):
            ...
            if block in self._slha_blocks:
                if d != self._slha_blocks[block]:
                    logger.critical("Incompatible entries for SLHA block %s" % block)
