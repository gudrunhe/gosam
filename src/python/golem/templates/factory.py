        ...
        if class_name == "Verbatim":
            ...
            try:
                ...
                if class_name == "Model":
                    ...
                    if "in_particles" in opts and "out_particles" in opts:
                        in_particles = opts["in_particles"]
                        out_particles = opts["out_particles"]
                        template.add_kinematics_parameters(in_particles, out_particles)
                elif class_name == "Integrals":
                    if (
                        "loopcache" not in opts
                    ...
                    template = golem.templates.integrals.IntegralsTemplate(f_template)
                    template.setup(
                        loopcache,
                        loopcache_tot,
                        in_particles,
                    ...
                    template = golem.templates.kinematics.KinematicsTemplate(f_template)
                    template.init_kinematics(
                        conf, in_particles, out_particles, tree_signs, heavy_quarks, helicity_map, ct_signs
                    )
                elif class_name == "OLP":
                    if (
                        "contract" not in opts
                        or "subprocesses" not in opts
