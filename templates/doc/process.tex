...
\noindent This process consists of [% @select topolopy.count.tree
 ...
 @end @select %] [% @if generate_nnlo_virt %] and [% @select topolopy.count.nnlo_docu 
 @case 0 %] no NNLO diagrams. [%
 @case 1 %] one NNLO diagram. [%
 @else %] [% topolopy.count.nnlo_docu %] NNLO diagrams. [%
 @end @select %] [% @end @if %] GoSam has identified [% @select count groups
 ...
 @if generate_uv_counterterms
   %] There are [% @select topolopy.count.ct
 @case 0 %]no counterterms[%
 @case 1 %]one counter term[%
 @else %][%topolopy.count.ct %] counter terms[%
 @end @select %] from UV-renormalisation.[%
 @end @if %]
\end{abstract}
\newpage
\tableofcontents
...
\end{longtable}
[%   @else %]
[%   @if eval DIAG .mod. 2 .eq. 0 %]\\[% @else %]&[%@end @if%]
[%   @end @if %]
[% @end @for %][%
   @end @with%][%
@end @if pyxodraw %][%
...
[% @if pyxodraw %]
[% @with modules pyxo2virt.tex path=process %]
[%  @for elements topolopy.keep.nnlo_virt var=DIAG index=DIAGINDEX %]
[%    @if is_first%]\begin{longtable}{cc}
%\endfirsthead
%\endhead[% @end @if %]
%---#[ 2loop diagram[% DIAG %]:
...
[% @if extensions noformopt %][% @else %]FORM~\cite{Kuipers:2012rf,Vermaseren:2000nd} and [% @end @if %]
