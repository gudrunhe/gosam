![% ' vim: ts=3:sw=3:expandtab:syntax=golem
 %]
module     [% process_name asprefix=\_ %]ninja
   ! This file has been generated for ninja 
   use ninjago_module, only: ki_nin
   use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config
   implicit none
   private[%
...
subroutine     ninja_reduce_group[% grp %](scale2,tot,totr,ok)
   ...
   use [% process_name asprefix=\_ %]d[% DIAG %]l1, only: numerator_diagram[% DIAG %] => numerator_ninja
   use [% process_name asprefix=\_ %]d[% DIAG %]l121, only: numerator_tmu_diagram[% DIAG %] => numerator_tmu
   use [% process_name asprefix=\_ %]d[% DIAG %]l131, only: numerator_t3_diagram[% DIAG %] => numerator_t3
   use [% process_name asprefix=\_ %]d[% DIAG %]l132, only: numerator_t2_diagram[% DIAG %] => numerator_t2[%
         @end @for %]
...
!-----#] subroutine ninja_reduce_group[% grp %]:[%
   @end @for %]
!---#] reduce groups with ninja:
end module [% process_name asprefix=\_ %]ninja
