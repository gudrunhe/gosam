...
subroutine     ninja_reduce_group[% grp %](scale2,tot,totr,ok)
   ...
   use [% process_name asprefix=\_ %]d[% DIAG %]l121_qp, only: numerator_tmu_diagram[% DIAG %] => numerator_tmu
   use [% process_name asprefix=\_ %]d[% DIAG %]l131_qp, only: numerator_t3_diagram[% DIAG %] => numerator_t3
   use [% process_name asprefix=\_ %]d[% DIAG %]l132_qp, only: numerator_t2_diagram[% DIAG %] => numerator_t2[%
         @end @for %]
   ...
   @for diagrams group=grp var=DIAG index=DIAGIDX idxshift=1 %][%
      ...
      @end @if %]
         ...
         call quadninja_diagram(numerator_diagram[% DIAG %], numerator_t3_diagram[% DIAG %], numerator_t2_diagram[% DIAG %], &
...
!-----#] subroutine ninja_reduce_group[% grp %]:[%
   @end @for %]
!---#] reduce groups with ninja:
end module [% process_name asprefix=\_ %]ninja_qp
