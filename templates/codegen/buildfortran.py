...
abbfile.write('module     [% process_name asprefix=\_
            %]abbrevd'+diag[% @if use_order_names %]+'_[% trnco %]'[% @end @if %][% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'\n')
abbfile.write('   use config, only: ki\n')
...
abbfile.write('      use config, only: deltaOS, &\n')
abbfile.write('     &    logfile, debug_nlo_diagrams\n')
abbfile.write('      use [% process_name asprefix=\_ %]kinematics\n')
abbfile.write('      use SpinorBrackets\n')
abbfile.write('      use model\n')
...
abbfile.write('   end subroutine\n')
abbfile.write('end module [% process_name asprefix=\_ %]abbrevd'+diag[% @if use_order_names %]+'_[% trnco %]'[% @end @if %][% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'\n')

[% @if extension quadruple %]
abbfile_qp.write('module     [% process_name asprefix=\_
            %]abbrevd'+diag[% @if use_order_names %]+'_[% trnco %]'[% @end @if %][% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'_qp\n')
abbfile_qp.write('   use config, only: ki => ki_qp\n')
...
abbfile_qp.write('      use config, only: deltaOS, &\n')
abbfile_qp.write('     &    logfile, debug_nlo_diagrams\n')
abbfile_qp.write('      use [% process_name asprefix=\_ %]kinematics_qp\n')
abbfile_qp.write('      use SpinorBrackets\n')
abbfile_qp.write('      use model_qp\n')
...
abbfile_qp.write('   end subroutine\n')
abbfile_qp.write('end module [% process_name asprefix=\_ %]abbrevd'+diag[% @if use_order_names %]+'_[% trnco %]'[% @end @if %][% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'_qp\n')
[% @end @if extension quadruple %]
...
[% @end @if extension quadruple %]
if int(heli) == -1:
    shutil.move(abb_tmpname,'abbrevd'+diag+'[% @if use_order_names %]_[% trnco %][% @end @if %].f90')
else:
    shutil.move(abb_tmpname,'abbrevd'+diag+'[% @if use_order_names %]_[% trnco %][% @end @if %]h'+heli+'.f90')
shutil.move(f90_tmpname,diag_name + '.f90')
[% @if extension quadruple %]
if int(heli) == -1:
    shutil.move(abb_tmpname_qp,'abbrevd'+diag+'[% @if use_order_names %]_[% trnco %][% @end @if %]_qp.f90')
else:
    shutil.move(abb_tmpname_qp,'abbrevd'+diag+'[% @if use_order_names %]_[% trnco %][% @end @if %]h'+heli+'_qp.f90')
