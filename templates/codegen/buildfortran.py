...
parser.add_option("-A", "--NOABB", dest="abb",
                  action="store_false", default=True,
                  help="do not create abbrev*.f90")

parser.add_option("-N", "--NODIAG", dest="acc",
                  action="store_false", default=True,
                  help="do not create d*h*l1.f90")
...
if acc:
	...
	txtfilename = diag_name + '.txt'
else:
	datfilename = options.input.split('.')[0] + '.dat'
	txtfilename = options.input.split('.')[0] + '.txt'
# import txt file
txt_lines=[]
dotprod_spva=[]
dotprod_sp=[]
...
outdict=translatefile(txtfilename,config)
...
if abb:
	...
	abbfile.write('   complex(ki), dimension('+str(abb_max)+'), public :: abb'[% @select abbrev.level @case diagram %]+diag[% @end @select %]+'\n')[%
	@select abbrev.level
		@case diagram %]
	abbfile.write('   complex(ki), public :: R2d'+diag+'\n')[%
		@case helicity %]
	abbfile.write('   complex(ki), public :: R2Sum\n')[%
	@end @select %]
	abbfile.write('\n')
	...
	abbfile.write('          write (logfile,*) "<result name=\'r2\' index=\''+diag+'\' value=\'", &\n')[%
		@case helicity %]
	abbfile.write('          write (logfile,*) "<result name=\'r2sum\' value=\'", &\n')[%
	@end @select %][%
	@select abbrev.level
		@case diagram  %]
	...
	abbfile.write('end module [% process_name asprefix=\_ %]abbrevd'+diag+'h'+heli+'\n')[%
		@case helicity %]
	abbfile.write('end module [% process_name asprefix=\_ %]abbrevh'+heli+'\n')[%
	@end @select %]

if acc:
	abbrev_filename_prefix = [%
	@select abbrev.level
		@case diagram
	%]'abbrevd'+diag+'h'+heli[%
		@case helicity
	...
	f90file.write('      use [% process_name asprefix=\_ %]'+abbrev_filename_prefix+'\n')
	f90file.write('      implicit none\n')
...
if abb:
	...
	shutil.move(abb_tmpname,'abbrevd'+diag+'h'+heli+'.f90')[%
		@case helicity %]
	shutil.move(abb_tmpname,'abbrevh'+heli+'.f90')[%
	@end @select %]
