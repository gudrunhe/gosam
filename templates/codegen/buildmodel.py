...
modelfile.write('   use [$ @if internal OLP_MODE $][$ @else $][$ process_name asprefix=\_ $][$ @end @if $]config, only: ki[$ @if extension quadruple $], ki_qp[$ @end @if $]')
modelfile.write(', &\n')
...
modelfile.write('   use [$ @if internal OLP_MODE $][$ @else $][$ process_name asprefix=\_ $][$ @end @if $]model_qp, only: set_parameter_qp => set_parameter\n')[$ @end @if $]
...
modelfile_qp.write('\n')
modelfile_qp.write('   real(ki), parameter :: sqrt2 = &\n')
modelfile_qp.write('      &1.414213562373095048801688724209698078&\n')
modelfile_qp.write('      &5696718753769480731766797379_ki\n')
modelfile_qp.write('   real(ki), parameter :: sqrt3 = &\n')
modelfile_qp.write('      &1.732050807568877293527446341505872366&\n')
modelfile_qp.write('      &9428052538103806280558069795_ki\n')[$
      @for parameters $][$
         @select type
            @case R $]
modelfile_qp.write('   real(ki) :: [$$_$] = [$ real convert=float format=%24.15f_ki $]\n')[$
            @case C $]
modelfile_qp.write('   complex(ki) :: [$$_$] = ([$ real convert=float format=%24.15f_ki $], [$ imag convert=float format=%24.15f_ki $])\n')[$
            @case RP $]
modelfile_qp.write('   real(ki),parameter :: [$$_$] = [$ real convert=float format=%24.15f_ki $]\n')[$
            @case CP $]
modelfile_qp.write('   complex(ki), parameter :: [$$_$] = ([$ real convert=float format=%24.15f_ki $], [$imag convert=float format=%24.15f_ki $])\n')[$
         @end @select type $][$
      @end @for parameters $][$
      @for functions $][$
         @select type
            @case R $]
modelfile_qp.write('   real(ki) :: [$$_$]\n')[$
            @case C $]
modelfile_qp.write('   complex(ki) :: [$$_$]\n')[$
         @end @select type $][$
      @end @for functions $]
modelfile_qp.write('   integer, parameter, private :: line_length = [$buffer_length$]\n')
#
#   ' what is our longest extra name ?
#   ' 0   0    1    1    2    2
#   ' 1---5----0----5----0----5
...
#   ' ==> the longest is 24
#
modelfile_qp.write('   integer, parameter, private :: name_length = max([$name_length$],24)\n')
modelfile_qp.write('   character(len=name_length), dimension([$ count R C $]) :: names = (/&\n')[$
   @for parameters R C  $]
modelfile_qp.write('      & "[$ $_ $][$ alignment $]"')[$
         @if is_last $]
modelfile_qp.write('/)\n')
   [$ @else $]
modelfile_qp.write(', &\n')
[$ @end @if $][$
   @end @for parse_names $]
modelfile_qp.write('   character(len=1), dimension([$ len_comment_chars $]) :: cc = (/')[$
   @for comment_chars $]
modelfile_qp.write(' \'[$$_$]\'')[$ @if is_last $]
modelfile_qp.write('/)\n')[$ @else $]
modelfile_qp.write(',')[$
   @end @if $][$
   @end @for$]
modelfile_qp.write('\n')
[$ @if ewchoose $]
modelfile_qp.write('   ! for automatic choosing the right EW scheme in set_parameters\n')
modelfile_qp.write('   integer, private :: choosen_ew_parameters ! bit-set of EW parameters\n')
modelfile_qp.write('   character(len=5), private, dimension(6) :: ew_parameters = &\n')
modelfile_qp.write('          &(/\'mW   \',&\n')
modelfile_qp.write('          &  \'mZ   \',&\n')
modelfile_qp.write('          &  \'alpha\',&\n')
modelfile_qp.write('          &  \'GF   \',&\n')
modelfile_qp.write('          &  \'sw   \',&\n')
modelfile_qp.write('          &  \'e    \'/)\n')
modelfile_qp.write('   integer, private :: choosen_ew_parameters_count = 0 ! bitset of EW parameters\n')
modelfile_qp.write('   integer, private :: orig_ewchoice = -1 ! saves the original ewchoice\n')
[$@end @if$]
modelfile_qp.write("   private :: digit, parsereal, names, cc\n")
modelfile_qp.write("\n")
modelfile_qp.write("contains\n")
modelfile_qp.write("\n")

modelfile_qp.write("!---#[ print_parameter:\n")
modelfile_qp.write("   ! Print current parameters / setup to stdout or output_unit\n")
modelfile_qp.write("   subroutine   print_parameter(verbose,output_unit)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      logical, intent(in), optional :: verbose\n")
modelfile_qp.write("      integer, intent(in), optional :: output_unit\n")
modelfile_qp.write("      logical :: is_verbose\n")
modelfile_qp.write("      integer :: unit\n")
modelfile_qp.write("\n")
modelfile_qp.write("      real(ki), parameter :: pi = 3.14159265358979323846264&\n")
modelfile_qp.write("     &3383279502884197169399375105820974944592307816406286209_ki\n")
modelfile_qp.write("      is_verbose = .false.\n")
modelfile_qp.write("      if(present(verbose)) then\n")
modelfile_qp.write("          is_verbose = verbose\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("\n")
modelfile_qp.write("      unit = 6 ! stdout\n")
modelfile_qp.write("      if(present(output_unit)) then\n")
modelfile_qp.write("          unit = output_unit\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("\n")
modelfile_qp.write("\n")
...
modelfile_qp.write("   else if(reduction_interoperation.eq.3) then\n")
...
modelfile_qp.write("      else if(reduction_interoperation_rescue.eq.3) then\n")
modelfile_qp.write("         write(unit,'(A1,1x,A15,A5)') \"#\", \"    --> rescue \", \"PJFRY\"\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("   end if\n")
[$ @if ewchoose $]
modelfile_qp.write("    write(unit,'(A1,1x,A11,I2)') \"#\", \"ewchoice = \", ewchoice\n")[$
@end @if$][$
@select modeltype @case sm smdiag smehc sm_complex smdiag_complex smehc smdiagehc $]
modelfile_qp.write("   write(unit,'(A1,1x,A27)') \"#\", \"--- PARAMETERS Overview ---\"\n")
modelfile_qp.write("   write(unit,'(A1,1x,A22)') \"#\", \"Boson masses & widths:\"\n")
modelfile_qp.write("   write(unit,'(A1,1x,A5,G23.16)') \"#\", \"mZ = \", mZ\n")
modelfile_qp.write("   write(unit,'(A1,1x,A5,G23.16)') \"#\", \"mW = \", mW\n")
modelfile_qp.write("   write(unit,'(A1,1x,A5,G23.16)') \"#\", \"mH = \", mH\n")
modelfile_qp.write("   write(unit,'(A1,1x,A5,G23.16)') \"#\", \"wZ = \", wZ\n")
modelfile_qp.write("   write(unit,'(A1,1x,A5,G23.16)') \"#\", \"wW = \", wW\n")
modelfile_qp.write("   write(unit,'(A1,1x,A5,G23.16)') \"#\", \"wH = \", wH\n")
modelfile_qp.write("   write(unit,'(A1,1x,A20)') \"#\", \"Active light quarks:\"\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"Nf    =\", Nf\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"Nfgen =\", Nfgen\n")
modelfile_qp.write("   write(unit,'(A1,1x,A23)') \"#\", \"Fermion masses & width:\"\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mU   = \", mU\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mD   = \", mD\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mS   = \", mS\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mC   = \", mC\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mB   = \", mB\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mBMS = \", mBMS\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"wB   = \", wB\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mT   = \", mT\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"wT   = \", wT\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"me   = \", me\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mmu  = \", mmu\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"mtau = \", mtau\n")
modelfile_qp.write("   write(unit,'(A1,1x,A7,G23.16)') \"#\", \"wtau = \", wtau\n")
modelfile_qp.write("   write(unit,'(A1,1x,A14)') \"#\", \"Couplings etc.:\"\n")
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"alphaS = \", gs*gs/4._ki/pi\n")
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"gs     = \", gs\n")
[$@if ewchoose $]
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"alpha  = \", alpha\n")
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"e      = \", e\n")
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"GF     = \", GF\n")
[$@end @if $]
[$@select modeltype @case sm_complex smdiag_complex $]
modelfile_qp.write("   write(unit,'(A1,1x,A9,\"(\",G23.16,G23.16,\")\")') \"#\", \"sw     = \", sw\n")
modelfile_qp.write("   write(unit,'(A1,1x,A9,\"(\",G23.16,G23.16,\")\")') \"#\", \"sw2    = \", sw*sw\n")
[$@else $]
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"sw     = \", sw\n")
modelfile_qp.write("   write(unit,'(A1,1x,A9,G23.16)') \"#\", \"sw2    = \", sw*sw\n")
[$ @end @select$]
modelfile_qp.write("   if(is_verbose) then\n")
[$@end @select $]
modelfile_qp.write("   write(unit,'(A1,1x,A21)') \"#\", \"--- ALL PARAMETERS ---\"\n")
[$@for parameters $][$
   @select type @case R $]
modelfile_qp.write("   write(unit,'(A1,1x,A11,G23.16)') \"#\", \"[$$_ convert=str format=%-5s$]= \", [$$_$]\n")
[$@case C $]
modelfile_qp.write("   write(unit,'(A1,1x,A11,\"(\",G23.16,G23.16,\")\")') \"#\", \"[$$_ convert=str format=%-5s$]= \", [$$_$]\n")
[$@case RP $]
modelfile_qp.write("   write(unit,'(A1,1x,A11,G23.16,\"const.\")') \"#\", \"[$$_ convert=str format=%-5s$]= \", [$$_$]\n")
[$@case CP $]
modelfile_qp.write("   write(unit,'(A1,1x,A11,\"(\",G23.16,G23.16,\")\",\"const.\")') \"#\", \"[$$_ convert=str format=%-5s$]= \", [$$_$]\n")
[$@end @select type $][$
@end @for parameters $]
modelfile_qp.write("   if(is_verbose) then\n")
[$
@for functions $][$
   @select type @case R $]
modelfile_qp.write("   write(unit,'(A1,1x,A11,G23.16,\"calc.\")') \"#\", \"[$$_ convert=str format=%-5s$]= \", [$$_$]\n")
[$@case C $]
modelfile_qp.write("   write(unit,'(A1,1x,A11,\"(\",G23.16,G23.16,\")\",\" calc.\")') \"#\", \"[$$_ convert=str format=%-5s$]= \", [$$_$]\n")
[$@end @select type $][$
@end @for functions $]
modelfile_qp.write("   end if\n")
[$@select modeltype @case sm smdiag smehc sm_complex smdiag_complex smdiagehc $]
modelfile_qp.write("   end if\n")
[$@end @select$]
modelfile_qp.write("   write(unit,'(A1,1x,A25)') \"#\", \"-------------------------\"\n")
modelfile_qp.write("   end subroutine\n")
modelfile_qp.write("!---#] print_parameter:\n")

modelfile_qp.write("   function     digit(ch, lnr) result(d)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      character(len=1), intent(in) :: ch\n")
modelfile_qp.write("      integer, intent(in) :: lnr\n")
modelfile_qp.write("      integer :: d\n")
modelfile_qp.write("      d = -1\n")
modelfile_qp.write("      select case(ch)\n")[$
         @for repeat 10 $]
modelfile_qp.write("         case(\'[$$_$]\')\n")
modelfile_qp.write("            d = [$$_$]\n")[$
         @end @for $]
modelfile_qp.write("         case default\n")
modelfile_qp.write("            write(*,'(A21,1x,I5)') 'invalid digit in line', lnr\n")
modelfile_qp.write("         end select\n")
modelfile_qp.write("   end function digit\n")
modelfile_qp.write("\n")
modelfile_qp.write("   function     parsereal(str, ierr, lnr) result(num)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      character(len=*), intent(in) :: str\n")
modelfile_qp.write("      integer, intent(out) :: ierr\n")
modelfile_qp.write("      integer, intent(in) :: lnr\n")
modelfile_qp.write("      integer, dimension(0:3,0:4), parameter :: DFA = &\n")
modelfile_qp.write("      & reshape( (/1,  1,  2, -1,   & ! state = 0\n")
modelfile_qp.write("      &            1, -1,  2,  3,   & ! state = 1\n")
modelfile_qp.write("      &            2, -1, -1,  3,   & ! state = 2\n")
modelfile_qp.write("      &            4,  4, -1, -1,   & ! state = 3\n")
modelfile_qp.write("      &            4, -1, -1, -1/), (/4, 5/))\n")
modelfile_qp.write("      real(ki) :: num\n")
modelfile_qp.write("      integer :: i, expo, ofs, state, cclass, d, s1, s2\n")
modelfile_qp.write("      num = 0.0_ki\n")
modelfile_qp.write("      expo = 0\n")
modelfile_qp.write("      state = 0\n")
modelfile_qp.write("      ofs = 0\n")
modelfile_qp.write("      s1 = 1\n")
modelfile_qp.write("      s2 = 1\n")
modelfile_qp.write("      d = -1\n")
modelfile_qp.write("      cclass = -1\n")
modelfile_qp.write("      do i=1,len(str)\n")
modelfile_qp.write("         select case(str(i:i))\n")
modelfile_qp.write("         case('_', '''', ' ')\n")
modelfile_qp.write("            cycle\n")
modelfile_qp.write("         case('+', '-')\n")
modelfile_qp.write("            cclass = 1\n")
modelfile_qp.write("         case('.')\n")
modelfile_qp.write("            cclass = 2\n")
modelfile_qp.write("         case('E', 'e')\n")
modelfile_qp.write("            cclass = 3\n")
modelfile_qp.write("         case default\n")
modelfile_qp.write("            cclass = 0\n")
modelfile_qp.write("            d = digit(str(i:i), lnr)\n")
modelfile_qp.write("            if (d .eq. -1) then\n")
modelfile_qp.write("               ierr = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end if\n")
modelfile_qp.write("         end select\n")
modelfile_qp.write("         if (cclass .eq. 0) then\n")
modelfile_qp.write("            select case(state)\n")
modelfile_qp.write("            case(0, 1)\n")
modelfile_qp.write("               num = 10.0_ki * num + d\n")
modelfile_qp.write("            case(2)\n")
modelfile_qp.write("               num = 10.0_ki * num + d\n")
modelfile_qp.write("               ofs = ofs - 1\n")
modelfile_qp.write("            case(4)\n")
modelfile_qp.write("               expo = 10 * expo + d\n")
modelfile_qp.write("            end select\n")
modelfile_qp.write("         elseif ((cclass .eq. 1) .and. (str(i:i) .eq. '-')) then\n")
modelfile_qp.write("            if (state .eq. 0) then\n")
modelfile_qp.write("               s1 = -1\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               s2 = -1\n")
modelfile_qp.write("            endif\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         ! Advance in the DFA\n")
modelfile_qp.write("         state = DFA(cclass, state)\n")
modelfile_qp.write("         if (state < 0) then\n")
modelfile_qp.write("            write(*,'(A21,1x,A1,1x,A7,I5)') 'invalid position for', &\n")
modelfile_qp.write("            & str(i:i), 'in line', lnr\n")
modelfile_qp.write("            ierr = 1\n")
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("      end do\n")
modelfile_qp.write("      num = s1 * num * 10.0_ki**(ofs + s2 * expo)\n")
modelfile_qp.write("      ierr = 0\n")
modelfile_qp.write("   end function parsereal\n")
modelfile_qp.write("\n")
modelfile_qp.write("   subroutine     parseline(line,stat,line_number)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      character(len=*), intent(in) :: line\n")
modelfile_qp.write("      integer, intent(out) :: stat\n")
modelfile_qp.write("      integer, intent(in), optional :: line_number\n")
modelfile_qp.write("\n")
modelfile_qp.write("      character(len=line_length) :: rvalue, ivalue, value\n")
modelfile_qp.write("      character(len=name_length) :: name\n")
modelfile_qp.write("      real(ki) :: re, im\n")
modelfile_qp.write("      integer :: idx, icomma, idx1, idx2, lnr, nidx, ierr, pdg\n")
modelfile_qp.write("\n")
modelfile_qp.write("      if(present(line_number)) then\n")
modelfile_qp.write("         lnr = line_number\n")
modelfile_qp.write("      else\n")
modelfile_qp.write("         lnr = 0\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("\n")
modelfile_qp.write("      idx = scan(line, '=', .false.)\n")
modelfile_qp.write("      if (idx .eq. 0) then\n")
modelfile_qp.write("         if(present(line_number)) then\n")
modelfile_qp.write("            write(*,'(A13,1x,I5)') 'error at line', line_number\n")
modelfile_qp.write("         else\n")
modelfile_qp.write("            write(*,'(A18)') 'error in parseline'\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         stat = 1\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("      name = adjustl(line(1:idx-1))\n")
modelfile_qp.write("      value = adjustl(line(idx+1:len(line)))\n")
modelfile_qp.write("      idx = scan(value, ',', .false.)\n")
modelfile_qp.write("\n")
modelfile_qp.write("      if (name .eq. \"renormalisation\") then\n")
modelfile_qp.write("         re = parsereal(value, ierr, lnr)\n")
modelfile_qp.write("         if (ierr .ne. 0) then\n")
modelfile_qp.write("            stat = 1\n")
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         renormalisation = int(re)\n")
modelfile_qp.write("      elseif (name .eq. \"EFTcount\") then\n")
modelfile_qp.write("         re = parsereal(value, ierr, lnr)\n")
modelfile_qp.write("         if (ierr .ne. 0) then\n")
modelfile_qp.write("            stat = 1\n")
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         EFTcount = int(re)\n")
modelfile_qp.write("      elseif (name .eq. \"nlo_prefactors\") then\n")
modelfile_qp.write("         re = parsereal(value, ierr, lnr)\n")
modelfile_qp.write("         if (ierr .ne. 0) then\n")
modelfile_qp.write("            stat = 1\n")
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         nlo_prefactors = int(re)\n")
modelfile_qp.write("      elseif (name .eq. \"deltaOS\") then\n")
modelfile_qp.write("         re = parsereal(value, ierr, lnr)\n")
modelfile_qp.write("         if (ierr .ne. 0) then\n")
modelfile_qp.write("            stat = 1\n")
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         deltaOS = int(re)\n")
modelfile_qp.write("      elseif (name .eq. \"reduction_interoperation\") then\n")
modelfile_qp.write("         re = parsereal(value, ierr, lnr)\n")
modelfile_qp.write("         if (ierr .ne. 0) then\n")
modelfile_qp.write("            stat = 1\n")
modelfile_qp.write("            return\n")
...
modelfile_qp.write("         end do\n")
modelfile_qp.write("         if (idx .gt. 0) then\n")
modelfile_qp.write("            rvalue = value(1:idx-1)\n")
modelfile_qp.write("            ivalue = value(idx+1:len(value))\n")
modelfile_qp.write("            re = parsereal(rvalue, ierr, lnr)\n")
modelfile_qp.write("            if (ierr .ne. 0) then\n")
modelfile_qp.write("               stat = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end if\n")
modelfile_qp.write("            im = parsereal(ivalue, ierr, lnr)\n")
modelfile_qp.write("            if (ierr .ne. 0) then\n")
modelfile_qp.write("               stat = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end if\n")
...
modelfile_qp.write("               return\n")
modelfile_qp.write("            end if\n")
modelfile_qp.write("            im = 0.0_ki\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         select case (nidx)\n")[$
         @for parameters R C $]
modelfile_qp.write("         case([$index$])\n")
modelfile_qp.write("            [$ $_ $] = ")[$
         @select type
         @case C$]
modelfile_qp.write("cmplx(re, im, ki)\n")[$
         @else $]
modelfile_qp.write("re\n")[$
         @end @select $][$
         @end @for $]
modelfile_qp.write("         end select\n")[$
@if has_slha_locations $][$
   @for slha_blocks lower dimension=1 $]
modelfile_qp.write('      elseif (name(1:[$ eval 1 + .len. $_ $]).eq."[$ $_ $](") then\n')
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         endif\n")
modelfile_qp.write("         read(name([$ eval 2 + .len. $_ $]:idx-1),*, iostat=ierr) pdg\n")
modelfile_qp.write("         if (ierr.ne.0) then\n")
modelfile_qp.write("            write(*,*) \"Not an integer:\", name([$ eval 2 + .len. $_ $]:idx-1)\n")
modelfile_qp.write("            if(present(line_number)) then\n")
modelfile_qp.write("               write(*,'(A13,1x,I5)') 'error at line', line_number\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A18)') 'error in parseline'\n")
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         select case(pdg)\n")[$
      @for slha_entries $]
modelfile_qp.write("            case([$index$])\n")
modelfile_qp.write("               [$ $_ $] = parsereal(value, ierr, lnr)\n")[$
      @end @for $]
modelfile_qp.write("            case default\n")
modelfile_qp.write("               write(*,'(A20,1x,I10)') \"Cannot set [$ $_ $] for code:\", pdg\n")
modelfile_qp.write("               stat = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("         end select\n")[$
   @end @for $][$
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         endif\n")
modelfile_qp.write("         icomma = scan(name(1:idx), ',', .false.)\n")
modelfile_qp.write("         if (icomma.eq.0) then\n")
modelfile_qp.write("            if(present(line_number)) then\n")
modelfile_qp.write("               write(*,'(A13,1x,I5)') 'error at line', line_number\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A18)') 'error in parseline'\n")
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         endif\n")
modelfile_qp.write("         read(name([$ eval 2 + .len. $_ $]:icomma-1),*, iostat=ierr) idx1\n")
modelfile_qp.write("         if (ierr.ne.0) then\n")
modelfile_qp.write("            write(*,*) \"Not an integer:\", name([$ eval 2 + .len. $_ $]:icomma-1)\n")
modelfile_qp.write("            if(present(line_number)) then\n")
modelfile_qp.write("               write(*,'(A13,1x,I5)') 'error at line', line_number\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A18)') 'error in parseline'\n")
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         read(name(icomma+1:idx-1),*, iostat=ierr) idx2\n")
modelfile_qp.write("         if (ierr.ne.0) then\n")
modelfile_qp.write("            write(*,*) \"Not an integer:\", name(icomma+1:idx-1)\n")
modelfile_qp.write("            if(present(line_number)) then\n")
modelfile_qp.write("               write(*,'(A13,1x,I5)') 'error at line', line_number\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A18)') 'error in parseline'\n")
...
modelfile_qp.write("         end if\n")[$
         @end @if$]
modelfile_qp.write("               if(idx1.eq.[$index1$]) then\n")[$
         @for slha_entries index=index2 $]
            [$
            @if is_first $]
modelfile_qp.write("if(idx2.eq.[$index2$]) then\n")            
            [$ @else $]
modelfile_qp.write(" elseif(idx2.eq.[$index2$]) then\n")[$
            @end @if $]
modelfile_qp.write("               [$ $_ $] = parsereal(value, ierr, lnr)\n")[$
            @if is_last $]
modelfile_qp.write("            end if\n")[$
...
modelfile_qp.write("         end if\n")[$
   ...
   @end @for $][$
@end @if $]
modelfile_qp.write('      elseif (name(1:2).eq.\"m(\" .or. name(1:2).eq.\"w(\") then\n')
modelfile_qp.write("         idx = scan(name, ')', .false.)\n")
modelfile_qp.write("         if (idx.eq.0) then\n")
modelfile_qp.write("            if(present(line_number)) then\n")
modelfile_qp.write("               write(*,'(A13,1x,I5)') 'error at line', line_number\n")
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         endif\n")
modelfile_qp.write("         read(name(3:idx-1),*, iostat=ierr) pdg\n")
modelfile_qp.write("         if (ierr.ne.0) then\n")
modelfile_qp.write('            write(*,*) "pdg is not an integer:", name(3:idx-1)\n')
modelfile_qp.write("            if(present(line_number)) then\n")
modelfile_qp.write("               write(*,'(A13,1x,I5)') 'error at line', line_number\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A18)') 'error in parseline'\n")
...
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         if (name(1:1).eq.\"m\") then\n")
modelfile_qp.write("            ! set mass according to PDG code\n")
modelfile_qp.write("            select case(pdg)\n")[$
@if has_slha_locations $][$
   @for slha_blocks lower $][$
      @select $_ @case mass $][$
...
@end @if $]
modelfile_qp.write("            case default\n")
modelfile_qp.write("               write(*,'(A20,1x,I10)') \"Cannot set mass for PDG code:\", pdg\n")
modelfile_qp.write("               stat = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end select\n")
modelfile_qp.write("         else\n")
modelfile_qp.write("            ! set width according to PDG code\n")
modelfile_qp.write("            select case(pdg)\n")[$
@if has_slha_locations $][$
   @for slha_blocks lower $][$
      @select $_ @case decay $][$
...
@end @if $]
modelfile_qp.write("            case default\n")
modelfile_qp.write("               write(*,'(A20,1x,I10)') \"Cannot set width for PDG code:\", pdg\n")
modelfile_qp.write("               stat = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end select\n")
modelfile_qp.write("         endif\n")
modelfile_qp.write("      else\n")
modelfile_qp.write("         write(*,'(A20,1x,A20)') 'Unrecognized option:', name\n")
modelfile_qp.write("         stat = 1\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("      stat = 0\n")
modelfile_qp.write("   end subroutine parseline\n")
modelfile_qp.write("\n")
modelfile_qp.write("   subroutine     parse(aunit)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      integer, intent(in) :: aunit\n")
modelfile_qp.write("      character(len=line_length) :: line\n")
modelfile_qp.write("      integer :: ios, lnr\n")
modelfile_qp.write("      lnr = 0\n")
modelfile_qp.write("      loop1: do\n")
modelfile_qp.write("         read(unit=aunit,fmt='(A[$buffer_length$])',iostat=ios) line\n")
modelfile_qp.write("         if(ios .ne. 0) exit\n")
modelfile_qp.write("         lnr = lnr + 1\n")
modelfile_qp.write("         line = adjustl(line)\n")
modelfile_qp.write("         if (line .eq. '') cycle loop1\n")
modelfile_qp.write("         if (any(cc .eq. line(1:1))) cycle loop1\n")
modelfile_qp.write("\n")
modelfile_qp.write("         call parseline(line,ios,lnr)\n")
modelfile_qp.write("         if(ios .ne. 0) then\n")
modelfile_qp.write("            write(*,'(A44,I2,A1)') &\n")
modelfile_qp.write("            & 'Error while reading parameter file in parse(', aunit, ')'\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("      end do loop1\n")
modelfile_qp.write("   end subroutine parse\n")[$
@if has_slha_locations $]
modelfile_qp.write("!---#[ SLHA READER:\n")
modelfile_qp.write("   subroutine     read_slha(ch, ierr)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      integer, intent(in) :: ch\n")
modelfile_qp.write("      integer, intent(out), optional :: ierr\n")
modelfile_qp.write("\n")
modelfile_qp.write("      integer :: lnr, i, l, ofs, ios\n")
modelfile_qp.write("      character(len=255) :: line\n")
modelfile_qp.write("\n")
modelfile_qp.write("      integer :: block\n")
modelfile_qp.write("\n")
modelfile_qp.write("      ofs = iachar('A') - iachar('a')\n")
modelfile_qp.write("\n")
modelfile_qp.write("      lnr = 0\n")
modelfile_qp.write("      loop1: do\n")
modelfile_qp.write("         read(unit=ch,fmt='(A[$buffer_length$])',iostat=ios) line\n")
modelfile_qp.write("         if(ios .ne. 0) exit\n")
modelfile_qp.write("         lnr = lnr + 1\n")
modelfile_qp.write("\n")
modelfile_qp.write("         i = scan(line, '#', .false.)\n")
modelfile_qp.write("         if (i .eq. 0) then\n")
modelfile_qp.write("            l = len_trim(line)\n")
modelfile_qp.write("         else\n")
modelfile_qp.write("            l = i - 1\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("\n")
modelfile_qp.write("         if (l .eq. 0) cycle loop1\n")
modelfile_qp.write("\n")
modelfile_qp.write("         ucase: do i = 1, l\n")
modelfile_qp.write("            if (line(i:i) >= 'a' .and. line(i:i) <= 'z') then\n")
modelfile_qp.write("               line(i:i) = achar(iachar(line(i:i))+ofs)\n")
modelfile_qp.write("            end if\n")
modelfile_qp.write("         end do ucase\n")
modelfile_qp.write("\n")
modelfile_qp.write("         if (line(1:1) .eq. 'B') then\n")
modelfile_qp.write("            if (line(1:5) .eq. 'BLOCK') then\n")
modelfile_qp.write("               line = adjustl(line(6:l))\n")
modelfile_qp.write("               do i=1,l\n")
modelfile_qp.write("                 if (line(i:i) <= ' ') exit\n")
modelfile_qp.write("               end do\n")
modelfile_qp.write("               l = i\n")[$
         @for slha_blocks upper $]
               [$
            @if is_first $][$ @else $]
modelfile_qp.write("           else")[$
            @end @if
               $]
modelfile_qp.write("           if (\"[$ $_ $]\" .eq. line(1:l)) then\n")
modelfile_qp.write("                  block = [$ index $]\n")[$
            @if is_last $]
modelfile_qp.write("               else\n")
modelfile_qp.write("                  block = -1\n")
modelfile_qp.write("               end if\n")[$
            @end @if $][$
         @end @for $]
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A37,I5)') \"Illegal statement in SLHA file, line \", lnr\n")
modelfile_qp.write("               if (present(ierr)) ierr = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end if\n")[$
         @for slha_blocks lower $][$
            @select $_ @case decay $]
modelfile_qp.write("         elseif (line(1:1) .eq. 'D') then\n")
modelfile_qp.write("            if (line(1:5) .eq. 'DECAY') then\n")
modelfile_qp.write("               line = adjustl(line(6:l))\n")
modelfile_qp.write("               call read_slha_line_decay(line, i)\n")
modelfile_qp.write("               block = 2\n")
modelfile_qp.write("            else\n")
modelfile_qp.write("               write(*,'(A37,I5)') \"Illegal statement in SLHA file, line \", lnr\n")
modelfile_qp.write("               if (present(ierr)) ierr = 1\n")
modelfile_qp.write("               return\n")
modelfile_qp.write("            end if\n")[$
         ...
         @end @for $]
modelfile_qp.write("         else\n")
modelfile_qp.write("            ! read a parameter line\n")
modelfile_qp.write("            select case(block)\n")[$
         @for slha_blocks lower $]
modelfile_qp.write("            case([$ index $])\n")
modelfile_qp.write("               call read_slha_block_[$ $_ $](line(1:l), i)\n")
modelfile_qp.write("               if (i .ne. 0) then\n")
modelfile_qp.write("                  if (present(ierr)) ierr = 1\n")
modelfile_qp.write("                  write(*,'(A44,I5)') &\n")
modelfile_qp.write("                  & \"Unrecognized line format in SLHA file, line \", lnr\n")
modelfile_qp.write("                  return\n")
modelfile_qp.write("               end if\n")[$
         @end @for $]
modelfile_qp.write("            case default\n")
modelfile_qp.write("               cycle loop1\n")
modelfile_qp.write("            end select\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("      end do loop1\n")
modelfile_qp.write("      if (present(ierr)) ierr = 0\n")
modelfile_qp.write("   end subroutine read_slha\n")[$
   @for slha_blocks lower dimension=1 $][$
      @select $_ @case decay $]
modelfile_qp.write("   subroutine read_slha_block_[$ $_ $](line, ierr)\n")
modelfile_qp.write("   !  This subroutine reads the 'branching ratios' of\n")
modelfile_qp.write("   !  the slha file: these are just thrown away\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      character(len=*), intent(in) :: line\n")
modelfile_qp.write("      integer, intent(out), optional :: ierr\n")
modelfile_qp.write("      integer :: idx1,idx2,ioerr,nda\n")
modelfile_qp.write("      real(ki) :: value\n")
modelfile_qp.write("      read(line,*,iostat=ioerr) value, nda, idx1, idx2\n")
modelfile_qp.write("      if (ioerr .ne. 0) then\n")
modelfile_qp.write("         if (present(ierr)) ierr = 1\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("      if (present(ierr)) ierr = 0\n")
modelfile_qp.write("   end subroutine read_slha_block_[$ $_ $]\n")
modelfile_qp.write("   subroutine read_slha_line_[$ $_ $](line, ierr)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      character(len=*), intent(in) :: line\n")
modelfile_qp.write("      integer, intent(out), optional :: ierr\n")[$
      @for slha_entries index=idx1$][$
         @if is_first $]
modelfile_qp.write("      integer :: idx1,ioerr\n")
modelfile_qp.write("      real(ki) :: value\n")
modelfile_qp.write("\n")
modelfile_qp.write("      read(line,*,iostat=ioerr) idx1, value\n")
modelfile_qp.write("      if (ioerr .ne. 0) then\n")
modelfile_qp.write("         if (present(ierr)) ierr = 1\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      end if\n")
modelfile_qp.write("      select case(idx1)\n")[$
         @end @if is_first $]
modelfile_qp.write("      case([$ idx1 $])\n")
modelfile_qp.write("         [$ $_ $] = value\n")[$
         @if is_last $]
modelfile_qp.write("      end select\n")[$
         @end @if is_last $][$
      @end @for$]
modelfile_qp.write("      if (present(ierr)) ierr = 0\n")
modelfile_qp.write("   end subroutine read_slha_line_[$ $_ $]\n")[$
   @else $]
modelfile_qp.write("   subroutine read_slha_block_[$ $_ $](line, ierr)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      character(len=*), intent(in) :: line\n")
...
modelfile_qp.write("      integer, intent(out), optional :: ierr\n")[$
      @for slha_entries index=idx1$][$
         @if is_first $]
modelfile_qp.write("      integer :: idx1, idx2, ioerr\n")
modelfile_qp.write("      real(ki) :: value\n")
modelfile_qp.write("\n")
modelfile_qp.write("      read(line,*,iostat=ioerr) idx1, idx2, value\n")
modelfile_qp.write("      if (ioerr .ne. 0) then\n")
modelfile_qp.write("         if (present(ierr)) ierr = 1\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      end if\n")
...
modelfile_qp.write("      select case(idx1)\n")[$
         @end @if is_first $]
modelfile_qp.write("      case([$ idx1 $])\n")
modelfile_qp.write("         select case(idx2)\n")[$
         @for slha_entries index=idx2 $]
modelfile_qp.write("         case([$ idx2 $])\n")
modelfile_qp.write("            [$ $_ $] = value\n")[$
         @end @for $]
modelfile_qp.write("         end select\n")[$
         @if is_last $]
...
modelfile_qp.write("!---#] SLHA READER:\n")[$
@end @if has_slha_locations $]
modelfile_qp.write("!---#[ subroutine set_parameter\n")
modelfile_qp.write("   recursive subroutine set_parameter(name, re, im, ierr)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      real(ki), parameter :: pi = 3.14159265358979323846264&\n")
modelfile_qp.write("     &3383279502884197169399375105820974944592307816406286209_ki\n")
modelfile_qp.write("      character(len=*), intent(in) :: name\n")
modelfile_qp.write("      real(ki), intent(in) :: re, im\n")
modelfile_qp.write("      integer, intent(out) :: ierr\n")
modelfile_qp.write("\n")
modelfile_qp.write("      integer :: err, pdg, nidx, idx\n")
modelfile_qp.write("      complex(ki) :: tmp\n")
modelfile_qp.write("\n")
modelfile_qp.write("      logical :: must_be_real\n")
modelfile_qp.write("      must_be_real = .false.\n")
modelfile_qp.write("      ierr = 1 ! OK\n")
modelfile_qp.write("\n")[$
@select modeltype @case sm smdiag smehc smdiagehc sm_complex smdiag_complex $][$
@if gs_not_one $]
modelfile_qp.write("      if (name.eq.\"aS\" .or. name.eq.\"alphaS\" .or. name.eq.\"alphas\") then\n")
modelfile_qp.write("         gs = 2.0_ki*sqrt(pi)*sqrt(re)\n")
modelfile_qp.write("         must_be_real = .true.\n")
modelfile_qp.write("      else")[$
@else $]
modelfile_qp.write("     ")[$
@end @if $][$
@if ewchoose $][$ @if alpha_not_one $]
modelfile_qp.write("if (name.eq.\"alphaEW\" .or. name.eq.\"alpha\") then\n")
modelfile_qp.write("         alpha = re\n")
modelfile_qp.write("         must_be_real = .true.\n")
modelfile_qp.write("      else")
[$ @end @if $][$ @end @if$][$
@select modeltype @case sm sm_complex smehc $]
modelfile_qp.write("if (name.eq.\"VV12\") then\n")
modelfile_qp.write("         call set_parameter(\"VUD\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV23\") then\n")
modelfile_qp.write("         call set_parameter(\"VUS\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV25\") then\n")
modelfile_qp.write("         call set_parameter(\"VUB\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV14\") then\n")
modelfile_qp.write("         call set_parameter(\"VCB\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV34\") then\n")
modelfile_qp.write("         call set_parameter(\"VCS\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV35\") then\n")
modelfile_qp.write("         call set_parameter(\"VCS\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV16\") then\n")
modelfile_qp.write("         call set_parameter(\"VTD\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV36\") then\n")
modelfile_qp.write("         call set_parameter(\"VTS\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"VV56\") then\n")
modelfile_qp.write("         call set_parameter(\"VTB\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      else")[$
@end @select $]
modelfile_qp.write("if (name.eq.\"Gf\") then\n")
modelfile_qp.write("         call set_parameter(\"GF\",re,im,ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("      elseif (name.eq.\"sw2\") then\n")
modelfile_qp.write("         tmp=sqrt(cmplx(re,im,ki))\n")
modelfile_qp.write("         call set_parameter(\"sw\",real(tmp,ki),aimag(tmp),ierr)\n")
modelfile_qp.write("         return\n")
modelfile_qp.write("     else")[$
@end @select $]
...
modelfile_qp.write("         end if\n")
modelfile_qp.write("         read(name(6:idx-1),*, iostat=err) pdg\n")
modelfile_qp.write("         if (err.ne.0) then\n")
...
modelfile_qp.write("            idx=len_trim(name)+1\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         read(name(7:idx-1),*, iostat=err) pdg\n")
modelfile_qp.write("         if (err.ne.0) then\n")
modelfile_qp.write("            ierr=0 !FAIL\n")
modelfile_qp.write("            return\n")
modelfile_qp.write("         end if\n")
...
modelfile_qp.write("            end select\n")[$
 @if has_slha_locations $][$
   @for slha_blocks upper dimension=1 name=blockname $][$
         @for slha_entries index=idx2 $]
modelfile_qp.write("     elseif (name .eq. \"[$ blockname  $]&&[$ idx2 $]\") then\n")
modelfile_qp.write("               must_be_real = .true.\n")
modelfile_qp.write("               [$ $_ $] = re\n")[$
         @end @for $][$
   @end @for $][$
@end @if $]
modelfile_qp.write("      elseif (name .eq. \"renormalisation\") then\n")
...
modelfile_qp.write("         end select\n")[$
@for parameter_alias real=re $]
modelfile_qp.write("      elseif (name .eq. \"[$ alias $]\") then\n")
modelfile_qp.write("            [$ $_ $] = [$ expr $]\n")
modelfile_qp.write("            must_be_real=.true.\n")[$
@end @for $]
modelfile_qp.write("     else\n")
...
modelfile_qp.write("     end if\n")
modelfile_qp.write("     if (must_be_real .and. im /= 0.0_ki .and. ierr.eq.1) then\n")
modelfile_qp.write("        ierr = 0 ! FAIL\n")
modelfile_qp.write("     end if\n")
modelfile_qp.write("\n")
[$ @if ewchoose $]
modelfile_qp.write("     if(any(ew_parameters .eq. name) .or. (name.eq.\"mass(23)\") .or. &\n")
modelfile_qp.write("        (name.eq.\"mass(24)\"))  then\n")
modelfile_qp.write("         do nidx=1,size(ew_parameters)\n")
modelfile_qp.write("            if (ew_parameters(nidx) .eq. name) exit\n")
modelfile_qp.write("         end do\n")
modelfile_qp.write("         if(name.eq.\"mass(23)\") then\n")
modelfile_qp.write("            nidx=2\n")
modelfile_qp.write("         elseif(name.eq.\"mass(24)\") then\n")
modelfile_qp.write("            nidx=1\n")
modelfile_qp.write("         end if\n")
modelfile_qp.write("         if (.not. btest(choosen_ew_parameters,nidx)) then\n")
modelfile_qp.write("            choosen_ew_parameters_count = choosen_ew_parameters_count + 1\n")
modelfile_qp.write("            choosen_ew_parameters = ibset(choosen_ew_parameters, nidx)\n")
[$ ' python program to calculate numbers below:\n")
 '   p=['mW','mZ','alpha','GF','sw','e']\n")
 '   print sum([2**(p.index(i.strip())+1) for i in \"GF,mW,mZ\".split(\",\")])\n")
 '   from itertools import combinations\n")
 '   ([sum([2**(p.index(i.strip())+1) for i in j]) for j in combinations(\"GF,mW,mZ\".split(\",\"),2)]) $]
modelfile_qp.write("            if (choosen_ew_parameters_count == 1) then\n")
modelfile_qp.write("               orig_ewchoice = ewchoice\n")
modelfile_qp.write("               if(ewchoice > 0) then\n")
modelfile_qp.write("                 select case(choosen_ew_parameters)\n")
modelfile_qp.write("                      case(2) ! mW\n")
modelfile_qp.write("                        if (ewchoice /= 1 .and. ewchoice /= 2 .and. &\n")
modelfile_qp.write("                            &   ewchoice /= 6) then\n")
modelfile_qp.write("                          ewchoice = 1\n")
modelfile_qp.write("                        end if\n")
modelfile_qp.write("                      case(4) ! mZ\n")
modelfile_qp.write("                        if (ewchoice /= 1 .and. ewchoice /= 2 .and. &\n")
modelfile_qp.write("                            &   ewchoice /= 6) then\n")
modelfile_qp.write("                          ewchoice = 1\n")
modelfile_qp.write("                        end if\n")
modelfile_qp.write("                      case(8) ! alpha\n")
modelfile_qp.write("                        if (ewchoice /= 2 .and. ewchoice /= 3 .and. &\n")
modelfile_qp.write("                            &   ewchoice /= 4 .and. ewchoice /= 5) then\n")
modelfile_qp.write("                          ewchoice = 2\n")
modelfile_qp.write("                        end if\n")
modelfile_qp.write("                      case(16) ! GF\n")
modelfile_qp.write("                        if (ewchoice /= 1 .and. ewchoice /= 4 .and. &\n")
modelfile_qp.write("                            &   ewchoice /= 8) then\n")
modelfile_qp.write("                          ewchoice = 1\n")
modelfile_qp.write("                        end if\n")
modelfile_qp.write("                     case(32) ! sw\n")
modelfile_qp.write("                        if (ewchoice /= 3 .and. ewchoice /= 4 .and. &\n")
modelfile_qp.write("                             &   ewchoice /= 7 .and. ewchoice /= 8) then\n")
modelfile_qp.write("                          ewchoice = 1\n")
modelfile_qp.write("                        end if\n")
[$@if e_not_one$]
modelfile_qp.write("                      case(64) ! e\n")
modelfile_qp.write("                        if (ewchoice < 6) then\n")
modelfile_qp.write("                           ewchoice = 6\n")
modelfile_qp.write("                        end if\n")
[$@end @if$]
modelfile_qp.write("                    end select\n")
modelfile_qp.write("                end if\n")
modelfile_qp.write("            elseif (choosen_ew_parameters_count == 2) then\n")
modelfile_qp.write("                if (choosen_ew_parameters == 18 .or. choosen_ew_parameters == 20 &\n")
modelfile_qp.write("                   & .or. choosen_ew_parameters == 6) then\n")
modelfile_qp.write("                   ewchoice = 1\n")
modelfile_qp.write("                elseif (choosen_ew_parameters == 10 .or. choosen_ew_parameters == 12) then\n")
modelfile_qp.write("                   ewchoice = 2\n")
modelfile_qp.write("                elseif (choosen_ew_parameters == 40 .or. choosen_ew_parameters == 36) then\n")
modelfile_qp.write("                   ewchoice = 3\n")
modelfile_qp.write("                elseif (choosen_ew_parameters == 24 .or. choosen_ew_parameters == 48) then\n")
modelfile_qp.write("                   ewchoice = 4\n")
modelfile_qp.write("                elseif (choosen_ew_parameters == 20) then\n")
modelfile_qp.write("                   ewchoice = 5\n")
[$@if e_not_one$]
modelfile_qp.write("                 elseif (choosen_ew_parameters == 66 .or. choosen_ew_parameters == 68) then\n")
modelfile_qp.write("                   ewchoice = 6\n")
modelfile_qp.write("                 elseif (choosen_ew_parameters == 96) then\n")
modelfile_qp.write("                   ewchoice = 7\n")
modelfile_qp.write("                 elseif (choosen_ew_parameters == 80) then\n")
modelfile_qp.write("                   ewchoice = 8\n")
[$@end @if$]
modelfile_qp.write("                 else\n")
modelfile_qp.write("                 ewchoice = orig_ewchoice\n")
modelfile_qp.write("                 write(*,'(A,1x,I2)') 'Unknown/Invalid EW scheme. Falling back to No.',&\n")
modelfile_qp.write("                                     ewchoice\n")
modelfile_qp.write("                 ierr = 0\n")
modelfile_qp.write("                end if\n")
modelfile_qp.write("            elseif (choosen_ew_parameters_count >= 4) then\n")
modelfile_qp.write("                 write(*,'(A,A,A)') 'EW parameter \"', name, '\" is already determined.'\n")
modelfile_qp.write("                 write(*,'(A)') 'New values are ignored.'\n")
modelfile_qp.write("                 write(*,'(A17,1x,I3)') 'Current EW choice:', ewchoice\n")
modelfile_qp.write("                 ierr = -1 ! IGNORE\n")
modelfile_qp.write("            elseif(choosen_ew_parameters_count == 3) then\n")
modelfile_qp.write("               select case(choosen_ew_parameters)\n")
modelfile_qp.write("                case(22) ! GF,mW,mZ -> e,sw\n")
modelfile_qp.write("                        ewchoice = 1\n")
modelfile_qp.write("                case(14) ! alpha, mW, mZ  -> e,sw\n")
modelfile_qp.write("                        ewchoice = 2\n")
modelfile_qp.write("                case(44) ! alpha, sw, mZ -> e, mW\n")
modelfile_qp.write("                        ewchoice = 3\n")
modelfile_qp.write("                case(56) ! alpha, sw, GF ->  e, mW\n")
modelfile_qp.write("                        ewchoice = 4\n")
modelfile_qp.write("                case(28) ! alpha, GF, mZ ->  e, mW, sw\n")
modelfile_qp.write("                        ewchoice = 5\n")
[$@if e_not_one$]
modelfile_qp.write("                case(70) ! e, mW, mZ -> sw\n")
modelfile_qp.write("                        ewchoice = 6\n")
modelfile_qp.write("                case(100) ! e, sw, mZ -> mW\n")
modelfile_qp.write("                        ewchoice = 7\n")
modelfile_qp.write("                case(112) ! e, sw, GF -> mW, mZ\n")
modelfile_qp.write("                        ewchoice = 8\n")
[$@end @if$]
modelfile_qp.write("                case default\n")
modelfile_qp.write("                 ewchoice = orig_ewchoice\n")
...
modelfile_qp.write("\n")
modelfile_qp.write("\n")
modelfile_qp.write("     call init_functions()\n")
modelfile_qp.write("      ! TODO init_color\n")
modelfile_qp.write("   end subroutine\n")
modelfile_qp.write("!---#] subroutine set_parameter\n")
...
modelfile_qp.write("!---#[ subroutine init_functions:\n")
modelfile_qp.write("   subroutine     init_functions()\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      complex(ki), parameter :: i_ = (0.0_ki, 1.0_ki)\n")
modelfile_qp.write("      real(ki), parameter :: pi = 3.14159265358979323846264&\n")
modelfile_qp.write("     &3383279502884197169399375105820974944592307816406286209_ki\n")[$
@select modeltype @case sm smdiag sm_complex smdiag_complex smehc smdiagehc $][$
@if ewchoose $]
modelfile_qp.write("      call ewschemechoice(ewchoice)\n")[$
@end @if $][$
@end @select $]
newfcs = re.sub(r"ctpolem([0-9]*)",r"(-\1)",outdict["Functions"])
newfcs = re.sub(r"ctpolep([0-9]*)",r"(\1)",newfcs)
newfcs = re.sub(r"([^a-zA-Z0-9]abs)_",r"\1",newfcs)
modelfile_qp.write("%s" % newfcs)
modelfile_qp.write("end subroutine init_functions\n")
modelfile_qp.write("!---#] subroutine init_functions:\n")
modelfile_qp.write("!---#[ utility functions for model initialization:\n")
modelfile_qp.write("   pure function ifpos(x0, x1, x2)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      real(ki), intent(in) :: x0, x1, x2\n")
modelfile_qp.write("      real(ki) :: ifpos\n")
modelfile_qp.write("\n")
modelfile_qp.write("      if (x0 > 0.0_ki) then\n")
modelfile_qp.write("         ifpos = x1\n")
modelfile_qp.write("      else\n")
modelfile_qp.write("         ifpos = x2\n")
modelfile_qp.write("      endif\n")
modelfile_qp.write("   end  function ifpos\n")
modelfile_qp.write("\n")
modelfile_qp.write("   pure function sort4(m1, m2, m3, m4, n)\n")
modelfile_qp.write("      implicit none\n")
modelfile_qp.write("      real(ki), intent(in) :: m1, m2, m3, m4\n")
modelfile_qp.write("      integer, intent(in) :: n\n")
modelfile_qp.write("      real(ki) :: sort4\n")
modelfile_qp.write("\n")
modelfile_qp.write("      real(ki), dimension(4) :: m\n")
modelfile_qp.write("      logical :: f\n")
modelfile_qp.write("      integer :: i\n")
modelfile_qp.write("      real(ki) :: tmp\n")
modelfile_qp.write("\n")
modelfile_qp.write("      m(1) = m1\n")
modelfile_qp.write("      m(2) = m2\n")
modelfile_qp.write("      m(3) = m3\n")
modelfile_qp.write("      m(4) = m4\n")
modelfile_qp.write("\n")
modelfile_qp.write("      ! Bubble Sort\n")
modelfile_qp.write("      do\n")
modelfile_qp.write("         f = .false.\n")
modelfile_qp.write("\n")
modelfile_qp.write("         do i=1,3\n")
modelfile_qp.write("            if (abs(m(i)) .gt. abs(m(i+1))) then\n")
modelfile_qp.write("               tmp = m(i)\n")
modelfile_qp.write("               m(i) = m(i+1)\n")
modelfile_qp.write("               m(i+1) = tmp\n")
modelfile_qp.write("               f = .true.\n")
modelfile_qp.write("            end if\n")
modelfile_qp.write("         end do\n")
modelfile_qp.write("\n")
modelfile_qp.write("         if (.not. f) exit\n")
modelfile_qp.write("      end do\n")
modelfile_qp.write("\n")
modelfile_qp.write("      sort4 = m(n)\n")
modelfile_qp.write("   end  function sort4\n")
modelfile_qp.write("!---#] utility functions for model initialization:\n")
[$ @select modeltype @case sm smdiag smehc smdiagehc $][$
@if ewchoose $]
modelfile_qp.write("!---#[ EW scheme choice:\n")[$
@if e_not_one $]
modelfile_qp.write("  subroutine ewschemechoice(ichoice)\n")
modelfile_qp.write("  implicit none\n")
modelfile_qp.write("  integer, intent(in) :: ichoice\n")
modelfile_qp.write("  real(ki), parameter :: pi = 3.14159265358979323846264&\n")
modelfile_qp.write(" &3383279502884197169399375105820974944592307816406286209_ki\n")
modelfile_qp.write("  select case (ichoice)\n")
modelfile_qp.write("        case (1)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("        sw = sqrt(1.0_ki-mW*mW/mZ/mZ)\n")
modelfile_qp.write("      ! GF, mW, sw --> e\n")
modelfile_qp.write("        e = mW*sw*sqrt(8.0_ki*GF/sqrt(2.0_ki))\n")
modelfile_qp.write("        case (2)\n")
modelfile_qp.write("      ! alpha --> e\n")
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("        sw = sqrt(1.0_ki-mW*mW/mZ/mZ)\n")
modelfile_qp.write("        case (3)\n")
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! sw, mZ --> mW\n")
modelfile_qp.write("        mW = mZ*sqrt(1.0_ki-sw*sw)\n")
modelfile_qp.write("        case (4)\n")
modelfile_qp.write("      ! alpha --> e\n")
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! GF, sw, alpha --> mW\n")
modelfile_qp.write("        mW = sqrt(alpha*pi/sqrt(2.0_ki)/GF) / sw\n")
modelfile_qp.write("      ! mW, sw --> mZ\n")
modelfile_qp.write("        mZ = mW / sqrt(1.0_ki-sw*sw)\n")
modelfile_qp.write("        case (5)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("        sw = sqrt(1.0_ki-mW*mW/mZ/mZ)\n")
modelfile_qp.write("        case (6)\n")
modelfile_qp.write("      ! mZ, sw --> mW\n")
modelfile_qp.write("        mW = mZ*sqrt(1-sw*sw)\n")
modelfile_qp.write("        case(7)\n")
modelfile_qp.write("      ! e, sw, GF --> mW\n")
modelfile_qp.write("        mW = e/2.0_ki/sw/sqrt(sqrt(2.0_ki)*GF)\n")
modelfile_qp.write("      ! mW, sw --> mZ\n")
modelfile_qp.write("        mZ = mW / sqrt(1.0_ki-sw*sw)\n")
modelfile_qp.write("        case(8)\n")
modelfile_qp.write("      ! alpha --> e\n")
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! GF, mZ, alpha --> mW\n")
modelfile_qp.write("      mW = sqrt(mZ*mZ/2.0_ki+sqrt(mZ*mZ*mZ*mZ/4.0_ki-pi*alpha*mZ*mZ/&\n")
modelfile_qp.write("     & sqrt(2.0_ki)/GF))\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("      sw = sqrt(1.0_ki-mW*mW/mZ/mZ)\n")
modelfile_qp.write("!        case default\n")
modelfile_qp.write("  end select\n")
...
modelfile_qp.write("  real(ki), parameter :: pi = 3.14159265358979323846264&\n")
modelfile_qp.write(" &3383279502884197169399375105820974944592307816406286209_ki\n")
modelfile_qp.write("  ! e is algebraically set to one, do not calculate it here\n")
modelfile_qp.write("  select case (ichoice)\n")
modelfile_qp.write("        case (1)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("        sw = sqrt(1.0_ki-mW*mW/mZ/mZ)\n")
...
modelfile_qp.write("        case (1)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("        sw = sqrt(1.0_ki-(mW*mW-i_*mW*wW)/(mZ*mZ-i_*mZ*wZ))\n")
modelfile_qp.write("      ! GF, mW, sw --> e\n")
modelfile_qp.write("        e = sqrt(mW*mW-i_*mW*wW)*sw*sqrt(8.0_ki*GF/sqrt(2.0_ki))\n")
modelfile_qp.write("        case (2)\n")
modelfile_qp.write("      ! alpha --> e\n")
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
...
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! sw, mZ --> mW\n")
modelfile_qp.write("        mW = sqrt(mZ*mZ-i_*mZ*wZ)*sqrt(1.0_ki-sw*sw)\n")
modelfile_qp.write("        case (4)\n")
modelfile_qp.write("      ! alpha --> e\n")
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! GF, sw, alpha --> mW\n")
modelfile_qp.write("        mW = sqrt(alpha*pi/sqrt(2.0_ki)/GF) / sw\n")
modelfile_qp.write("      ! mW, sw --> mZ\n")
modelfile_qp.write("        mZ = sqrt(mW*mW-i_*mW*wW) / sqrt(1.0_ki-sw*sw)\n")
modelfile_qp.write("        case (5)\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("        sw = sqrt(1.0_ki-(mW*mW-i_*mW*wW)/(mZ*mZ-i_*mZ*wZ))\n")
modelfile_qp.write("        case (6)\n")
...
modelfile_qp.write("        e = sqrt(4.0_ki*pi*alpha)\n")
modelfile_qp.write("      ! GF, mZ, alpha --> mW\n")
modelfile_qp.write("      mW = sqrt((mZ*mZ-i_*mZ*wZ)/2.0_ki+sqrt((mZ*mZ-i_*mZ*wZ)**2/4.0_ki-pi*alpha*(mZ*mZ-i_*mZ*wZ)/&\n")
modelfile_qp.write("     & sqrt(2.0_ki)/GF))\n")
modelfile_qp.write("      ! mW, mZ --> sw\n")
modelfile_qp.write("      sw = sqrt(1.0_ki-(mW*mW-i_*mW*wW)/(mZ*mZ-i_*mZ*wZ))\n")
modelfile_qp.write("!        case default\n")
