...
f90file.close()
[% @if extension quadruple %]
f90file_qp.write('module     [% process_name asprefix=\_%]'+diag_name[:-1]+'31_qp\n')
f90file_qp.write('   ! file: '+str(os.getcwd())+diag_name[:-1]+'31_qp.f90 \n')
...
for tpow in range(rk-nl+3,-1,-1):
    for mupow in range(0,rk-nl+3+1-tpow,2):
        f90file_qp.write('   integer, parameter :: ninjaidxt{0}mu{1} = {2}\n'.format(tpow,mupow,idxcount))
        idxcount = idxcount +1
f90file_qp.write('   public :: numerator_t3\n')
f90file_qp.write(' \n')
...
for lidx in expids:
	...
	f90file_qp.write('      implicit none \n')
	f90file_qp.write('      complex(ki), dimension(4), intent(in) :: ninjaA, ninjaE3, ninjaE4\n')
#	f90file_qp.write('      complex(ki), dimension(4), intent(in) :: Q\n')
	f90file_qp.write('      complex(ki), intent(in) :: ninjaP\n')
...
f90file_qp.write('      complex(ki), dimension(0:3), intent(in) :: a, b, c\n')
f90file_qp.write('      complex(ki), intent(in) :: param \n')
f90file_qp.write('      complex(ki), dimension(4) :: vecA, vecB, vecC\n')
...
if qshift=='0':
	f90file_qp.write('	     vecA(1:4) = ' + qsign + ' a(0:3)\n')
    ...
    f90file_qp.write('	     vecA(1:4) = ' + qsign + ' a(0:3) - qshift(1:4)\n')
    f90file_qp.write('	     vecB(1:4) = ' + qsign + ' b(0:3)\n')
    f90file_qp.write('	     vecC(1:4) = ' + qsign + ' c(0:3)\n')
...
f90file_qp.write('      call cond_t(epspow.eq.t1,brack_31,vecA,vecB,vecC,param,coeffs)\n')

if (len(expids)>=2):
    f90file_qp.write('      if (deg.le.(1+({0}))) return\n'.format(rk-nl))
    f90file_qp.write('      call cond_t(epspow.eq.t1,brack_32,vecA,vecB,vecC,param,coeffs)\n')

f90file_qp.write('   end subroutine numerator_t3 \n')
f90file_qp.write('!---#] subroutine numerator_t3: \n')


f90file_qp.write('end module     [% process_name asprefix=\_%]'+diag_name[:-1]+'31_qp\n')
f90file_qp.close()
[% @end @if extension quadruple %]
...
[% @if extension quadruple %]
shutil.move(tmpname_qp,diag_name[:-1]+'31_qp.f90')
[% @end @if extension quadruple %]
