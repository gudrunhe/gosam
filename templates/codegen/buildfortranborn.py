#! /usr/bin/env python3
# vim: ts=3:sw=3:expandtab

import sys
import os
import re
from optparse import OptionParser
from t2f import translatefile, getdata, postformat
from pythonin import parameters, kinematics, symbols, lambdafunc, dotproducts
import tempfile, shutil

config={'parameters' : parameters,
        'kinematics' : kinematics,
        'symbols' : symbols,
        'lambdafunc' : lambdafunc,
        'dotproducts' : dotproducts}

parser = OptionParser()

parser.add_option("-i", "--input", dest="input",
                  action="store", type="string",
                  help="input file", metavar="INPUT")

parser.add_option("-H", "--HELICITY", dest="helicity",
                  action="store", type="int",
                  help="helicity number", metavar="HELICITY")

(options, args) = parser.parse_args()

if not options.input:
    sys.exit("Error: no input file was found! Please specify one with the -i options.")

file_name= options.input.split('.')[0]
heli=options.helicity

print('----------------------------------')
print('Input file is:      %s' % file_name+'.txt')
print('Diagram written in: %s' % 'diagramsl0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %].f90')
print('----------------------------------')

txtfile = open(file_name+'.txt','r')
tmp_handle , tmpname = tempfile.mkstemp(suffix=".f90",prefix="gosam_tmp")
f90file = os.fdopen(tmp_handle,"w")
[% @if extension quadruple %]
tmp_handle_qp , tmpname_qp = tempfile.mkstemp(suffix="_qp.f90",prefix="gosam_tmp")
f90file_qp = os.fdopen(tmp_handle_qp,"w")
[% @end @if extension quadruple %]
datfilename = file_name + '.dat'

# import txt file
txt_lines=[]
abb_max=getdata(datfilename)['abbrev_terms']

[% @if enable_truncation_orders
%]outdict=translatefile(file_name+'.txt',config)[% @else
%]outdict=translatefile('born.txt',config)[% @end @if %]
f90file.write('module     [% process_name asprefix=\_ %]diagramsh'+str(heli)+'l0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %]\n')
f90file.write('   ! file: '+str(os.getcwd())+'diagramsl0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %].f90 \n')
f90file.write('   ! generator: buildfortranborn[% @if enable_truncation_orders %]_[% trnco %][% @end @if %].py \n')
f90file.write('   use [% process_name asprefix=\_ %]color, only: numcs\n')
f90file.write('   use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: ki\n')[%
@if internal CUSTOM_SPIN2_PROP %]
f90file.write('   use [% process_name asprefix=\_ %]custompropagator\n')[%
@end @if %]
f90file.write('\n')
f90file.write('   implicit none\n')
f90file.write('   private :: bornamplitude, reglog, yukctOS, yukctMSbar, massctOS\n')
f90file.write('\n')
f90file.write('   complex(ki), parameter :: i_ = (0.0_ki, 1.0_ki)\n')
f90file.write('   complex(ki), dimension(numcs), parameter :: zero_col = 0.0_ki\n')
f90file.write('   public :: amplitude, amplitude_Dym\n')
f90file.write('\n')
f90file.write('contains\n')
f90file.write('!---#[ function amplitude:\n')
f90file.write('   function amplitude()\n')
f90file.write('      implicit none\n')
f90file.write('      complex(ki), dimension(numcs) :: amplitude\n')
f90file.write('\n')
f90file.write('      amplitude = bornamplitude(.false., 0, -1.0_ki, 0)\n')
f90file.write('\n')
f90file.write('   end function     amplitude\n')
f90file.write('!---#] function amplitude:\n')
f90file.write('!---#[ function amplitude_Dym:\n')
f90file.write('   function amplitude_Dym(renorm_mode, scale2, eps)\n')
f90file.write('      implicit none\n')
f90file.write('      complex(ki), dimension(numcs) :: amplitude_Dym\n')
f90file.write('      real(ki) :: scale2\n')
f90file.write('      integer :: eps, renorm_mode\n')
f90file.write('\n')
f90file.write('      amplitude_Dym = bornamplitude(.true., renorm_mode, scale2, eps)\n')
f90file.write('\n')
f90file.write('   end function     amplitude_Dym\n')
f90file.write('!---#] function amplitude_Dym:\n')
f90file.write('!---#[ function bornamplitude:\n')
f90file.write('   function bornamplitude(renorm, renorm_mode, scale2, eps) result(amplitude)\n')
f90file.write('      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]model\n')
f90file.write('      use [% process_name asprefix=\_ %]kinematics\n')
f90file.write('      use [% process_name asprefix=\_ %]color\n')
f90file.write('      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: debug_lo_diagrams, &\n')
f90file.write('        & use_sorted_sum, renorm_logs, renorm_mqse, renorm_yukawa\n')
f90file.write('      use accu, only: sorted_sum\n')
f90file.write('      use [% process_name asprefix=\_ %]util, only: inspect_lo_diagram\n')
f90file.write('      implicit none\n')
f90file.write('      complex(ki), dimension(numcs) :: amplitude\n')
f90file.write('      complex(ki), dimension('+str(abb_max)+') :: abb\n')
f90file.write('!      complex(ki), dimension(2,numcs) :: diagrams\n')
f90file.write('      integer :: i, eps, renorm_mode\n')
f90file.write('      real(ki) :: CYUKAWA, CMASS, XCT, scale2\n')
f90file.write('      logical :: renorm, logs\n')
f90file.write('\n')
f90file.write('      amplitude(:) = 0.0_ki\n')
f90file.write('\n')
f90file.write('      if (renorm) then\n')
f90file.write('         logs = renorm_logs\n')
f90file.write('         XCT = 1.0_ki\n')
f90file.write('         select case (renorm_mode)\n')
f90file.write('            case (0)\n')
f90file.write('               ! No renormalistion. This case is only here for completeness and never actually used.\n')
f90file.write('               return\n')
f90file.write('            case (1)\n')
f90file.write('               if (renorm_yukawa) then\n')
f90file.write('                  CYUKAWA = 1.0_ki\n')
f90file.write('               else\n')
f90file.write('                  CYUKAWA = 0.0_ki\n')
f90file.write('               end if\n')
f90file.write('               if (renorm_mqse) then\n')
f90file.write('                  CMASS = 1.0_ki\n')
f90file.write('               else\n')
f90file.write('                  CMASS = 0.0_ki\n')
f90file.write('               end if\n')
f90file.write('            case (2)\n')
f90file.write('               CYUKAWA = 0.0_ki\n')
f90file.write('               CMASS = 1.0_ki\n')
f90file.write('            case (3)\n')
f90file.write('               ! Quark mass renormalisation the old way. This case is only here for completeness and never actually used.\n')
f90file.write('               return\n')
f90file.write('            case default\n')
f90file.write('               print *, \"ERROR: In function bornamplitude: unkown case for renorm_mode.\"\n')
f90file.write('               stop\n')
f90file.write('         end select\n')
f90file.write('      else\n')
f90file.write('         logs = .false.\n')
f90file.write('         XCT = 0.0_ki\n')
f90file.write('         CYUKAWA = 0.0_ki\n')
f90file.write('         CMASS = 0.0_ki\n')
f90file.write('      end if\n')
f90file.write('\n')
f90file.write('\n')
newabbr = re.sub(r"DELTAYUKOS([a-zA-Z0-9]*)",r"yukctOS(renorm,eps,logs,scale2/\1/\1)",outdict['Abbreviations'])
newabbr = re.sub(r"DELTAYUKMSbar([a-zA-Z0-9]*)",r"yukctMSbar(renorm,eps)",newabbr)
newabbr = re.sub(r"DELTAMASSOS([a-zA-Z0-9]*)",r"massctOS(renorm,eps,logs,scale2,\1)",newabbr)
f90file.write(newabbr)
f90file.write('\n')
f90file.write(outdict['Diagrams'])
f90file.write('\n')
f90file.write('      if (debug_lo_diagrams) then\n')
f90file.write('         write(*,*) "Using Born optimization, debug_lo_diagrams not implemented."\n')
f90file.write('      end if\n')
f90file.write('\n')
f90file.write('!      if (use_sorted_sum) then\n')
f90file.write('!         do i=1,numcs\n')
f90file.write('!            amplitude(i) = sorted_sum(diagrams(i))\n')
f90file.write('!         end do\n')
f90file.write('!      else\n')
f90file.write('!         do i=1,numcs\n')
f90file.write('!            amplitude(i) = sum(diagrams(i))\n')
f90file.write('!         end do\n')
f90file.write('!      end if\n')
f90file.write('   end function     bornamplitude\n')
f90file.write('!---#] function bornamplitude:\n')
f90file.write('!---#[ function yukctOS:\n')
f90file.write('   function yukctOS(renorm,eps,logs,r)\n')
f90file.write('      use [% process_name asprefix=\_ %]color, only: CF\n')
f90file.write('      implicit none\n')
f90file.write('      real(ki) :: yukctOS, r\n')
f90file.write('      integer :: eps\n')
f90file.write('      logical :: renorm, logs\n')
f90file.write('\n')
f90file.write('      if (.not.renorm) then\n')
f90file.write('         yukctOS = 0.0_ki\n')
f90file.write('         return\n')
f90file.write('      end if\n')
f90file.write('\n')
f90file.write('      select case (eps)\n')
f90file.write('         case(-1)\n')
f90file.write('            yukctOS = -1.5_ki*CF\n')
f90file.write('         case(0)\n')
f90file.write('            yukctOS = -[% @if extension dred %]2.5[% @else %]2.0[% @end @if %]_ki*CF\n')
f90file.write('            if (logs) then\n')
f90file.write('               yukctOS = yukctOS - 1.5_ki*CF*reglog(r)\n')
f90file.write('            end if\n')
f90file.write('         case default\n')
f90file.write('            print *, \"ERROR: In function yukctOS: unkown epsilon power.\"\n')
f90file.write('            stop\n')
f90file.write('      end select\n')
f90file.write('\n')
f90file.write('   end function     yukctOS\n')
f90file.write('!---#] function yukctOS:\n')
f90file.write('!---#[ function yukctMSbar:\n')
f90file.write('   function yukctMSbar(renorm,eps)\n')
f90file.write('      use [% process_name asprefix=\_ %]color, only: CF\n')
f90file.write('      implicit none\n')
f90file.write('      real(ki) :: yukctMSbar\n')
f90file.write('      integer :: eps\n')
f90file.write('      logical :: renorm\n')
f90file.write('\n')
f90file.write('      if (.not.renorm) then\n')
f90file.write('         yukctMSbar = 0.0_ki\n')
f90file.write('         return\n')
f90file.write('      end if\n')
f90file.write('\n')
f90file.write('      select case (eps)\n')
f90file.write('         case(-1)\n')
f90file.write('            yukctMSbar = -1.5_ki*CF\n')
f90file.write('         case(0)\n')
f90file.write('            yukctMSbar = -[% @if extension dred %]0.5[% @else %]0.0[% @end @if %]_ki*CF\n')
f90file.write('         case default\n')
f90file.write('            print *, \"ERROR: In function yukctMSbar: unkown epsilon power.\"\n')
f90file.write('            stop\n')
f90file.write('      end select\n')
f90file.write('\n')
f90file.write('   end function     yukctMSbar\n')
f90file.write('!---#] function yukctMSbar:\n')
f90file.write('!---#[ function massctOS:\n')
f90file.write('   function massctOS(renorm,eps,logs,scale2,m)\n')
f90file.write('      use [% process_name asprefix=\_ %]color, only: CF\n')
f90file.write('      implicit none\n')
f90file.write('      real(ki) :: massctOS, scale2, m\n')
f90file.write('      integer :: eps\n')
f90file.write('      logical :: renorm, logs\n')
f90file.write('\n')
f90file.write('      if (.not.renorm) then\n')
f90file.write('         massctOS = 0.0_ki\n')
f90file.write('         return\n')
f90file.write('      end if\n')
f90file.write('\n')
f90file.write('      select case (eps)\n')
f90file.write('         case(-1)\n')
f90file.write('            massctOS = -1.5_ki*CF\n')
f90file.write('         case(0)\n')
f90file.write('            massctOS = -[% @if extension dred %]2.5[% @else %]2.0[% @end @if %]_ki*CF\n')
f90file.write('            if (logs) then\n')
f90file.write('               massctOS = (massctOS - 1.5_ki*CF*reglog(scale2/m/m))\n')
f90file.write('            end if\n')
f90file.write('         case default\n')
f90file.write('            print *, \"ERROR: In function massctOS: unkown epsilon power.\"\n')
f90file.write('            stop\n')
f90file.write('      end select\n')
f90file.write('\n')
f90file.write('      massctOS = m*massctOS\n')
f90file.write('\n')
f90file.write('   end function     massctOS\n')
f90file.write('!---#] function massctOS:\n')
f90file.write('!---#[ function reglog:\n')
f90file.write('   function reglog(r)\n')
f90file.write('      implicit none\n')
f90file.write('      real(ki) :: reglog, r\n')
f90file.write('\n')
f90file.write('      if (r.le.0.0_ki) then\n')
f90file.write('         reglog = 0.0_ki\n')
f90file.write('      else\n')
f90file.write('         reglog = log(r)\n')
f90file.write('      end if\n')
f90file.write('\n')
f90file.write('   end function     reglog\n')
f90file.write('!---#] function reglog:\n')
f90file.write('end module [% process_name asprefix=\_ %]diagramsh'+str(heli)+'l0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %]\n')
f90file.close()
[% @if extension quadruple %]
f90file_qp.write('module     [% process_name asprefix=\_ %]diagramsh'+str(heli)+'l0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %]_qp\n')
f90file_qp.write('   ! file: '+str(os.getcwd())+'diagramsl0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %]_qp.f90 \n')
f90file_qp.write('   ! generator: buildfortranborn[% @if enable_truncation_orders %]_[% trnco %][% @end @if %].py \n')
f90file_qp.write('   use [% process_name asprefix=\_ %]color_qp, only: numcs\n')
f90file_qp.write('   use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: ki => ki_qp\n')[%
@if internal CUSTOM_SPIN2_PROP %]
f90file_qp.write('   use [% process_name asprefix=\_ %]custompropagator\n')[%
@end @if %]
f90file_qp.write('\n')
f90file_qp.write('   implicit none\n')
f90file_qp.write('   private :: bornamplitude, reglog, yukctOS, yukctMSbar, massctOS\n')
f90file_qp.write('\n')
f90file_qp.write('   complex(ki), parameter :: i_ = (0.0_ki, 1.0_ki)\n')
f90file_qp.write('   complex(ki), dimension(numcs), parameter :: zero_col = 0.0_ki\n')
f90file_qp.write('   public :: amplitude, amplitude_Dym\n')
f90file_qp.write('\n')
f90file_qp.write('contains\n')
f90file_qp.write('!---#[ function amplitude:\n')
f90file_qp.write('   function amplitude()\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      complex(ki), dimension(numcs) :: amplitude\n')
f90file_qp.write('\n')
f90file_qp.write('      amplitude = bornamplitude(.false., 0, -1.0_ki, 0)\n')
f90file_qp.write('\n')
f90file_qp.write('   end function     amplitude\n')
f90file_qp.write('!---#] function amplitude:\n')
f90file_qp.write('!---#[ function amplitude_Dym:\n')
f90file_qp.write('   function amplitude_Dym(renorm_mode, scale2, eps)\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      complex(ki), dimension(numcs) :: amplitude_Dym\n')
f90file_qp.write('      real(ki) :: scale2\n')
f90file_qp.write('      integer :: eps, renorm_mode\n')
f90file_qp.write('\n')
f90file_qp.write('      amplitude_Dym = bornamplitude(.true., renorm_mode, scale2, eps)\n')
f90file_qp.write('\n')
f90file_qp.write('   end function     amplitude_Dym\n')
f90file_qp.write('!---#] function amplitude_Dym:\n')
f90file_qp.write('!---#[ function bornamplitude:\n')
f90file_qp.write('   function bornamplitude(renorm, renorm_mode, scale2, eps) result(amplitude)\n')
f90file_qp.write('      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]model_qp\n')
f90file_qp.write('      use [% process_name asprefix=\_ %]kinematics_qp\n')
f90file_qp.write('      use [% process_name asprefix=\_ %]color_qp\n')
f90file_qp.write('      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: debug_lo_diagrams, &\n')
f90file_qp.write('        & use_sorted_sum, renorm_logs, renorm_mqse, renorm_yukawa\n')
f90file_qp.write('      use accu_qp, only: sorted_sum\n')
f90file_qp.write('      use [% process_name asprefix=\_ %]util_qp, only: inspect_lo_diagram\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      complex(ki), dimension(numcs) :: amplitude\n')
f90file_qp.write('      complex(ki), dimension('+str(abb_max)+') :: abb\n')
f90file_qp.write('!      complex(ki), dimension(2,numcs) :: diagrams\n')
f90file_qp.write('      integer :: i, eps, renorm_mode\n')
f90file_qp.write('      real(ki) :: CYUKAWA, CMASS, XCT, scale2\n')
f90file_qp.write('      logical :: renorm, logs\n')
f90file_qp.write('\n')
f90file_qp.write('      amplitude(:) = 0.0_ki\n')
f90file_qp.write('\n')
f90file_qp.write('      if (renorm) then\n')
f90file_qp.write('         logs = renorm_logs\n')
f90file_qp.write('         XCT = 1.0_ki\n')
f90file_qp.write('         select case (renorm_mode)\n')
f90file_qp.write('            case (0)\n')
f90file_qp.write('               ! No renormalistion. This case is only here for completeness and never actually used.\n')
f90file_qp.write('               return\n')
f90file_qp.write('            case (1)\n')
f90file_qp.write('               if (renorm_yukawa) then\n')
f90file_qp.write('                  CYUKAWA = 1.0_ki\n')
f90file_qp.write('               else\n')
f90file_qp.write('                  CYUKAWA = 0.0_ki\n')
f90file_qp.write('               end if\n')
f90file_qp.write('               if (renorm_mqse) then\n')
f90file_qp.write('                  CMASS = 1.0_ki\n')
f90file_qp.write('               else\n')
f90file_qp.write('                  CMASS = 0.0_ki\n')
f90file_qp.write('               end if\n')
f90file_qp.write('            case (2)\n')
f90file_qp.write('               CYUKAWA = 0.0_ki\n')
f90file_qp.write('               CMASS = 1.0_ki\n')
f90file_qp.write('            case (3)\n')
f90file_qp.write('               ! Quark mass renormalisation the old way. This case is only here for completeness and never actually used.\n')
f90file_qp.write('               return\n')
f90file_qp.write('            case default\n')
f90file_qp.write('               print *, \"ERROR: In function bornamplitude: unkown case for renorm_mode.\"\n')
f90file_qp.write('               stop\n')
f90file_qp.write('         end select\n')
f90file_qp.write('      else\n')
f90file_qp.write('         logs = .false.\n')
f90file_qp.write('         XCT = 0.0_ki\n')
f90file_qp.write('         CYUKAWA = 0.0_ki\n')
f90file_qp.write('         CMASS = 0.0_ki\n')
f90file_qp.write('      end if\n')
f90file_qp.write('\n')
f90file_qp.write('\n')
newabbr = re.sub(r"DELTAYUKOS([a-zA-Z0-9]*)",r"yukctOS(renorm,eps,logs,scale2/\1/\1)",outdict['Abbreviations'])
newabbr = re.sub(r"DELTAYUKMSbar([a-zA-Z0-9]*)",r"yukctMSbar(renorm,eps)",newabbr)
newabbr = re.sub(r"DELTAMASSOS([a-zA-Z0-9]*)",r"massctOS(renorm,eps,logs,scale2,\1)",newabbr)
f90file_qp.write(newabbr)
f90file_qp.write('\n')
f90file_qp.write(outdict['Diagrams'])
f90file_qp.write('\n')
f90file_qp.write('      if (debug_lo_diagrams) then\n')
f90file_qp.write('         write(*,*) "Using Born optimization, debug_lo_diagrams not implemented."\n')
f90file_qp.write('      end if\n')
f90file_qp.write('\n')
f90file_qp.write('!      if (use_sorted_sum) then\n')
f90file_qp.write('!         do i=1,numcs\n')
f90file_qp.write('!            amplitude(i) = sorted_sum(diagrams(i))\n')
f90file_qp.write('!         end do\n')
f90file_qp.write('!      else\n')
f90file_qp.write('!         do i=1,numcs\n')
f90file_qp.write('!            amplitude(i) = sum(diagrams(i))\n')
f90file_qp.write('!         end do\n')
f90file_qp.write('!      end if\n')
f90file_qp.write('   end function     bornamplitude\n')
f90file_qp.write('!---#] function bornamplitude:\n')
f90file_qp.write('!---#[ function yukctOS:\n')
f90file_qp.write('   function yukctOS(renorm,eps,logs,r)\n')
f90file_qp.write('      use [% process_name asprefix=\_ %]color_qp, only: CF\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      real(ki) :: yukctOS, r\n')
f90file_qp.write('      integer :: eps\n')
f90file_qp.write('      logical :: renorm, logs\n')
f90file_qp.write('\n')
f90file_qp.write('      if (.not.renorm) then\n')
f90file_qp.write('         yukctOS = 0.0_ki\n')
f90file_qp.write('         return\n')
f90file_qp.write('      end if\n')
f90file_qp.write('\n')
f90file_qp.write('      select case (eps)\n')
f90file_qp.write('         case(-1)\n')
f90file_qp.write('            yukctOS = -1.5_ki*CF\n')
f90file_qp.write('         case(0)\n')
f90file_qp.write('            yukctOS = -[% @if extension dred %]2.5[% @else %]2.0[% @end @if %]_ki*CF\n')
f90file_qp.write('            if (logs) then\n')
f90file_qp.write('               yukctOS = yukctOS - 1.5_ki*CF*reglog(r)\n')
f90file_qp.write('            end if\n')
f90file_qp.write('         case default\n')
f90file_qp.write('            print *, \"ERROR: In function yukctOS: unkown epsilon power.\"\n')
f90file_qp.write('            stop\n')
f90file_qp.write('      end select\n')
f90file_qp.write('\n')
f90file_qp.write('   end function     yukctOS\n')
f90file_qp.write('!---#] function yukctOS:\n')
f90file_qp.write('!---#[ function yukctMSbar:\n')
f90file_qp.write('   function yukctMSbar(renorm,eps)\n')
f90file_qp.write('      use [% process_name asprefix=\_ %]color_qp, only: CF\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      real(ki) :: yukctMSbar\n')
f90file_qp.write('      integer :: eps\n')
f90file_qp.write('      logical :: renorm\n')
f90file_qp.write('\n')
f90file_qp.write('      if (.not.renorm) then\n')
f90file_qp.write('         yukctMSbar = 0.0_ki\n')
f90file_qp.write('         return\n')
f90file_qp.write('      end if\n')
f90file_qp.write('\n')
f90file_qp.write('      select case (eps)\n')
f90file_qp.write('         case(-1)\n')
f90file_qp.write('            yukctMSbar = -1.5_ki*CF\n')
f90file_qp.write('         case(0)\n')
f90file_qp.write('            yukctMSbar = -[% @if extension dred %]0.5[% @else %]0.0[% @end @if %]_ki*CF\n')
f90file_qp.write('         case default\n')
f90file_qp.write('            print *, \"ERROR: In function yukctMSbar: unkown epsilon power.\"\n')
f90file_qp.write('            stop\n')
f90file_qp.write('      end select\n')
f90file_qp.write('\n')
f90file_qp.write('   end function     yukctMSbar\n')
f90file_qp.write('!---#] function yukctMSbar:\n')
f90file_qp.write('!---#[ function massctOS:\n')
f90file_qp.write('   function massctOS(renorm,eps,logs,scale2,m)\n')
f90file_qp.write('      use [% process_name asprefix=\_ %]color_qp, only: CF\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      real(ki) :: massctOS, scale2, m\n')
f90file_qp.write('      integer :: eps\n')
f90file_qp.write('      logical :: renorm, logs\n')
f90file_qp.write('\n')
f90file_qp.write('      if (.not.renorm) then\n')
f90file_qp.write('         massctOS = 0.0_ki\n')
f90file_qp.write('         return\n')
f90file_qp.write('      end if\n')
f90file_qp.write('\n')
f90file_qp.write('      select case (eps)\n')
f90file_qp.write('         case(-1)\n')
f90file_qp.write('            massctOS = -1.5_ki*CF\n')
f90file_qp.write('         case(0)\n')
f90file_qp.write('            massctOS = -[% @if extension dred %]2.5[% @else %]2.0[% @end @if %]_ki*CF\n')
f90file_qp.write('            if (logs) then\n')
f90file_qp.write('               massctOS = (massctOS - 1.5_ki*CF*reglog(scale2/m/m))\n')
f90file_qp.write('            end if\n')
f90file_qp.write('         case default\n')
f90file_qp.write('            print *, \"ERROR: In function massctOS: unkown epsilon power.\"\n')
f90file_qp.write('            stop\n')
f90file_qp.write('      end select\n')
f90file_qp.write('\n')
f90file_qp.write('      massctOS = m*massctOS\n')
f90file_qp.write('\n')
f90file_qp.write('   end function     massctOS\n')
f90file_qp.write('!---#] function massctOS:\n')
f90file_qp.write('!---#[ function reglog:\n')
f90file_qp.write('   function reglog(r)\n')
f90file_qp.write('      implicit none\n')
f90file_qp.write('      real(ki) :: reglog, r\n')
f90file_qp.write('\n')
f90file_qp.write('      if (r.le.0.0_ki) then\n')
f90file_qp.write('         reglog = 0.0_ki\n')
f90file_qp.write('      else\n')
f90file_qp.write('         reglog = log(r)\n')
f90file_qp.write('      end if\n')
f90file_qp.write('\n')
f90file_qp.write('   end function     reglog\n')
f90file_qp.write('!---#] function reglog:\n')
f90file_qp.write('end module [% process_name asprefix=\_ %]diagramsh'+str(heli)+'l0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %]_qp\n')
f90file_qp.close()
[% @end @if extension quadruple %]
txtfile.close()

### additional formatting for output files

postformat(tmpname)
[% @if extension quadruple %]
postformat(tmpname_qp)
[% @end @if extension quadruple %]
shutil.move(tmpname,'diagramsl0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %].f90')
[% @if extension quadruple %]
shutil.move(tmpname_qp,'diagramsl0[% @if enable_truncation_orders %]_[% trnco %][% @end @if %]_qp.f90')
[% @end @if extension quadruple %]
