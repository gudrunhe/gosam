...
matcher = re.compile(r"spv?[a|b](?:[k|l|e]\d+){2}")
matches = set()

vecs = [[%@for particles %] [% @if eval is_massive %] "l[%index%]", [% @else %] "k[%index%]", [% @end @if %] [% @end @for particles %]]

spa_set = set([[%
@for pairs ordered distinct %]"spa[%
...
spb_set = set([[%
@for pairs ordered distinct %]"spb[%
                 @if is_lightlike2 %]k[% @else %]l[% @end @if %][% index2
            %][% @if is_lightlike1 %]k[% @else %]l[% @end @if %][% index1
            %]",[%
@end @for %]])
      ...
      @if eval is_lightlike1 .and. ( 2spin1 .eq. 2 ) .and.
         ...
         @with eval 'k .rep. ( reference1 > 0 ) . 'l .rep. ( reference1 < 0 )
          . ( .abs. reference1 ) result=refvec1 %][%
            @with eval 'k .rep. ( reference2 > 0 ) . 'l .rep. ( reference2 < 0 )
          . ( .abs. reference2 ) result=refvec2
            %]"spae[% index1 %]e[% index2 %]": ("N[%index1%] * N[%index2%] * Spaa(k[%index1%], k[%index2%])",[%
                                                @if eval refvec2 .eq. ( 'k . index1 ) %]"0.0_ki"[%
                                                @else %]"N[%index1%] * N[%index2%] * Spaa(k[%index1%], [%refvec2%])"[%
                                                @end @if %], [% @if eval refvec1 .eq. refvec2 %]"0.0_ki"[%
...
for file in glob.glob("../helicity*/d*h*l?.txt") + glob.glob("../helicity*/born.txt"):
    with open(file, "r") as filehandle:
        matches |= set(matcher.findall(filehandle.read()))
...
""")
    ...
    for spa in spa_set & matches:
        p, q = re.search(r"spa([k|l]\d+)([k|l]\d+)", spa).group(1, 2)
        outfile.write(f"      {spa} = Spaa({p}, {q})\n")
    for spb in spb_set & matches:
        p, q = re.search(r"spb([k|l]\d+)([k|l]\d+)", spb).group(1, 2)
        outfile.write(f"      {spb} = Spbb({p}, {q})\n")
    for spva in spva_set & matches:
        p, q = re.search(r"spva([k|l]\d+)([k|l]\d+)", spva).group(1, 2)
        outfile.write(f"      {spva} = Spab3_vec({p}, {q})\n")
   ...
   @for particles lightlike vector initial %][%
      @with eval 'k .rep. ( reference > 0 ) . 'l .rep. ( reference < 0 )
         ...
         case(1)
            ...
            e[% index %] = Spab3_vec([%refvec%], k[%index%])/N[%index%]
         case(-1)
            flag[%index%] = .true.
            N[%index%] = sqrt2*Spbb(k[%index%],[%refvec%])
            e[% index %] = Spab3_vec(k[%index%], [%refvec%])/N[%index%]
         case default
...
""")
    ...
    for spa_e in set(spae_dict.keys()):
        polvec = re.search(r"(e\d+)", spa_e).group(1)
        vec = re.search(r"([k|l]\d+)", spa_e).group(1)
        index = int(polvec[1:])
        if f"spa{vec}{polvec}" == spa_e:
            spb_e = f"spb{polvec}{vec}"
        else:
    ...
    for spa_ee in set(spaee_dict.keys()):
        polvec1, polvec2 = re.search(r"spa(e\d+)(e\d+)", spa_ee).group(1, 2)
        ...
        if spa_ee in matches or spb_ee in matches:
            ...
            if(flag{index2}) then
               ...
               {spb_ee} = {spaee_dict[spa_ee][2]}
            else
               {spa_ee} = {spaee_dict[spa_ee][1]}
               {spb_ee} = {spaee_dict[spa_ee][3]}
            end if
         else
            if(flag{index2}) then
    ...
    for spva_e in set(spvae_dict.keys()):
        polvec = re.search(r"(e\d+)", spva_e).group(1)
    ...
    for spva_ee in set(spvaee_dict.keys()):
        polvec1, polvec2 = re.search(r"spva(e\d+)(e\d+)", spva_ee).group(1, 2)
        index1 = int(polvec1[1:])
        index2 = int(polvec2[1:])
        if spva_ee in matches or f"spvae{index2}e{index1}" in matches:
            ...
            if(flag{index2}) then
               ...
               {f"spvae{index2}e{index1}"} = {spvaee_dict[spva_ee][1]}
            else
               {spva_ee} = {spvaee_dict[spva_ee][2]}
               {f"spvae{index2}e{index1}"} = {spvaee_dict[spva_ee][3]}
            end if
         else
            if(flag{index2}) then
...
@end @if %]
    ...
    outfile.write("""end subroutine init_event
!---#] subroutine init_event:
...
!---#[ function Spbb
   pure function Spbb(k1, k2)
    ...
    !-----negative energy case
        ...
        Spbb = sign(1.0_ki, k1(0)*k2(0) - k1(1)*k2(1) - k1(2)*k2(2) - k1(3)*k2(3)) &
...
!---#[ function Spab3_vec
   pure function Spab3_vec(k1, k2)
         ...
         Spab3_vec(0) = f1*f2*((pr1/rt1)*(pl2/rt2) + rt1*rt2)
         Spab3_vec(1) = f1*f2*(rt1*rt2 - (pr1/rt1)*(pl2/rt2))
         Spab3_vec(2) = i_*f1*f2*((pr1/rt1)*rt2 - rt1*(pl2/rt2))
         Spab3_vec(3) = f1*f2*((pr1/rt1)*rt2 + rt1*(pl2/rt2))
