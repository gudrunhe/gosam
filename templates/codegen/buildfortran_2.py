...
abbfile.write('module     [% process_name asprefix=\_
            %]abbrevd'+diag+'_2'[% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'\n')
...
abbfile.write('   end subroutine\n')
abbfile.write('end module [% process_name asprefix=\_ %]abbrevd'+diag+'_2'[% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'\n')

[% @if extension quadruple %]
abbfile_qp.write('module     [% process_name asprefix=\_
            %]abbrevd'+diag+'_2'[% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'_qp\n')
...
abbfile_qp.write('   end subroutine\n')
abbfile_qp.write('end module [% process_name asprefix=\_ %]abbrevd'+diag+'_2'[% @if helsum %][% @else %]+'h'+heli[% @end @if %]+'_qp\n')
[% @end @if extension quadruple %]
...
[% @if extension quadruple %]
if int(heli) == -1:
    shutil.move(abb_tmpname_qp,'abbrevd'+diag+'_2_qp.f90')
else:
    shutil.move(abb_tmpname_qp,'abbrevd'+diag+'_2h'+heli+'_qp.f90')
