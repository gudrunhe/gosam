...
contains
   ...
   function     amplitude(logs,scale2) result(amp)
      ...
      use [% process_name asprefix=\_ %]color[% @if extension quadruple %]_qp[% @end @if %], only: TR, CA, numcs
...
@end @select %]
      ...
      @end @if %][%
      @if is_real %]
               Deltagwf(0) = Deltagwf(0) - num_gluons * 2.0_ki * TR / 3.0_ki * &
                  &            log(scale2/[% $_ %]**2)[%
      @end @if %][%
      @if is_complex %]
               Deltagwf(0) = Deltagwf(0) - num_gluons * 2.0_ki * TR / 3.0_ki * &
                  &            log(scale2/[% $_ %]/conjg([% $_ %]))[%
      @end @if %] [%
      @if is_last %]
               end if
            end if[%
      ...
      @end @for %]
            ...
            amp(-1,:) = amp(-1,:) + Deltagwf[% @if enable_truncation_orders %]_0[% @end @if %](-1)*amp0()
            amp(0,:) = amp(0,:) + Deltagwf[% @if enable_truncation_orders %]_0[% @end @if %](0)*amp0()
      ...
      @if is_ehc%]
               ...
               !amp(0,:) = amp(0,:) + (11.0_ki -2.0_ki/3.0_ki*log(scale2/mH**2)) * amp0
               amp(0,:) = amp(0,:) + (11.0_ki) * amp0()[%
      @end @if %][%
      @end @for %]
            end if[% 
      @end @if %]
