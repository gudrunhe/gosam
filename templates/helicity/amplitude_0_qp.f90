[% ' vim: ts=3:sw=3:expandtab:syntax=golem
 %]module    [% process_name asprefix=\_ %]amplitudeh[% helicity %]_0_qp
...
@else %][%
   ...
   use [% process_name asprefix=\_ %]ninja_0h[% helicity %]_qp[%
      @end @if %][%
   @end @select %][%
@end @if %][%
@end @if %]
...
@if generate_lo_diagrams %]
      ...
      %]diagramsh[%helicity%]l0_0_qp, only: amplitudel0 => amplitude[%
