[% ' vim: ts=3:sw=3:expandtab:syntax=golem
 %]module    [% process_name asprefix=\_ %]amplitudeh[% helicity %]_2
...
@else %][%
   ...
   use [% process_name asprefix=\_ %]ninja_2h[% helicity %][%
      @end @if %][%
   @end @select %][%
@end @if %][%
@end @if %]
...
@if generate_lo_diagrams %]
      ...
      %]diagramsh[%helicity%]l0_2, only: amplitudel0 => amplitude[%
   @if internal REQUIRE_FR5 %]
      use [% process_name asprefix=\_
      %]diagramsh[%helicity%]l0_2fr5, only: amplitudel0fr5 => amplitude[%
...
@else %][%
   ...
   @case diagram %][%
      ...
      use [% process_name asprefix=\_
      %]abbrevd[%$_%]_2h[%helicity%], only: init_abbrevd[%$_%] => init_abbrev[%
         @end @for %][%
      @end @for %][%
   @case group %][%
      @for groups var=grp %]
...
@end @select %][%
      ...
      use [% process_name asprefix=\_
      %]diagramscth[%helicity%]_2, only: samplitudect => samplitudect[%
      @end @if %]
      implicit none
      real(ki), intent(in) :: scale2
...
@end @if %][%
   @end @for groups %][%
@end @select %]
end module [% process_name asprefix=\_ %]amplitudeh[% helicity %]_2
