[% ' vim: ts=3:sw=3:expandtab:syntax=golem
 %]module     [% process_name asprefix=\_ %]samuraih[% helicity %]
...
function     numeval_group[% grp %](icut, Q, mu2) result(num)[%
   ...
   use [% process_name asprefix=\_ %]d[% DIAG %]h[% helicity 
     %]l1, only: numerator_d[% DIAG %] => numerator_samurai[%
         @end @for %][%
         @if use_flags_1 %]
...
subroutine     reduce_group[% grp %](scale2,tot,totr,ok)
   ...
   use [% process_name asprefix=\_ %]model[%
         ...
         @for diagrams group=grp var=DIAG idxshift=1 %]
   use [% process_name asprefix=\_ %]d[% DIAG %]h[% helicity 
     %]l1, only: numerator_diagram[% DIAG %] => numerator_samurai[%
         @end @for %]
...
!-----#] subroutine reduce_group[% grp %]:[%
   @end @for %]
!---#] reduce groups with samurai:
end module [% process_name asprefix=\_ %]samuraih[% helicity %]
