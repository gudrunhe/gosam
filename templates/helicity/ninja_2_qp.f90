![% ' vim: ts=3:sw=3:expandtab:syntax=golem
 %]
module     [% process_name asprefix=\_ %]ninja_2h[% helicity %]_qp
...
subroutine     ninja_reduce_group[% grp %](scale2,tot,totr,ok)
   ...
   use [% process_name asprefix=\_ %]d[% DIAG %]_2h[% helicity 
...
!-----#] subroutine ninja_reduce_group[% grp %]:[%
   @end @for %]
!---#] reduce groups with ninja:
end module [% process_name asprefix=\_ %]ninja_2h[% helicity %]_qp
