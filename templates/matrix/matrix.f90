...
@end @if %]
   ...
   use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]model, only: Nf, NC, sqrt2, init_functions[%
@if extension quadruple %]
...
@if generate_eft_counterterms %][%
@if eval topolopy.count.ct .gt. 0 %]
   public :: samplitudeeftct, samplitudeeftct_h[%
@end @if %][%
@end @if %][%
@if generate_yuk_counterterms %]
      ...
      case (1)
         ...
         if(corrections_are_qcd) then
            ...
            if (renorm_yukawa) then
               if (present(h)) then
                  amp(3) = amp(3) + samplitudeyukct_h(vecs, renorm_logs, scale2, -1, h)
                  amp(2) = amp(2) + samplitudeyukct_h(vecs, renorm_logs, scale2, 0, h)
               else
                  amp(3) = amp(3) + samplitudeyukct(vecs, renorm_logs, scale2, -1)
                  amp(2) = amp(2) + samplitudeyukct(vecs, renorm_logs, scale2, 0)
               end if
            end if
            [% @end @if generate_yuk_counterterms %]
         ...
         if (renorm_eftwilson) then
            if (present(h)) then
               ampct((/4,3,2/)) = samplitudeeftct_h(vecs, renorm_logs, scale2, h)
            else
               ampct((/4,3,2/)) = samplitudeeftct(vecs, renorm_logs, scale2)
            end if
...
@if eval topolopy.count.ct .gt. 0 %]
   !---#[ function samplitudeeftct :
   function     samplitudeeftct(vecs, logs, scale2) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics, only: init_event
      implicit none
      real(ki), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @end @for unique_helicity_mappings%]
   ...
   @end @if generate_lo_diagrams %]
   end function samplitudeeftct
   !---#] function samplitudeeftct :
   !---#[ function samplitudeeftct_h :
   function     samplitudeeftct_h(vecs, logs, scale2, h) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics, only: init_event
      implicit none
      real(ki), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @end @for helicities %]
   ...
   @end @if generate_lo_diagrams %]
   end function samplitudeeftct_h
   !---#] function samplitudeeftct_h :
[% @end @if %]
[% @end @if generate_eft_counterterms %]
[% @if generate_yuk_counterterms %]
   !---#[ function samplitudeyukct :
   function     samplitudeyukct(vecs,logs,scale2,eps) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics, only: init_event
      implicit none
      real(ki), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @for unique_helicity_mappings %]
     ...
     @if enable_truncation_orders %]
         ...
         case (0)
            ...
            amp0_1 = amplitude[% map.index %]l0_1()
            amp0_2 = amplitude[% map.index %]l0_2()
            ampct_0 = amplitude[% map.index %]yukct_0(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2(logs,scale2,eps)
            heli_amp = square(amp0_0, ampct_0 + ampct_1 + ampct_2) &
            & + square(amp0_1 + amp0_2, ampct_0)
         case (1)
            ! sigma(SM + dim6 X SM + dim6) without loopcounting
            amp0_0 = amplitude[% map.index %]l0_0()
            amp0_1 = amplitude[% map.index %]l0_1()
            ...
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2(logs,scale2,eps)
            heli_amp = square(amp0_0 + amp0_1 + amp0_2, ampct_0 + ampct_1 + ampct_2)
         case (2)
            ! sigma(SM X SM) + sigma(SM X dim6) with loopcounting
            amp0_0 = amplitude[% map.index %]l0_0()
            amp0_1 = amplitude[% map.index %]l0_1()
            ampct_0 = amplitude[% map.index %]yukct_0(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            heli_amp = square(amp0_0, ampct_0 + ampct_1) &
            & + square(amp0_1, ampct_0)
         case (3)
           ! sigma(SM + dim6 X SM + dim6) with loopcounting
            amp0_0 = amplitude[% map.index %]l0_0()
            amp0_1 = amplitude[% map.index %]l0_1()
            ampct_0 = amplitude[% map.index %]yukct_0(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            heli_amp = square(amp0_0 + amp0_1, ampct_0 + ampct_1)
         case (4)
            ! sigma(SM X dim6) without loopcounting
            amp0_0 = amplitude[% map.index %]l0_0()
            amp0_1 = amplitude[% map.index %]l0_1()
            ...
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2(logs,scale2,eps)
            heli_amp = square(amp0_0, ampct_1 + ampct_2) &
            & + square(amp0_1 + amp0_2, ampct_0)
         case (5)
            ! sigma(dim6 X dim6)  without loopcounting
            amp0_1 = amplitude[% map.index %]l0_1()
            ...
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2(logs,scale2,eps)
            heli_amp = square(amp0_1 + amp0_2, ampct_1 + ampct_2)
         case (6)
            ! sigma(SM X dim6) with loopcounting
            amp0_0 = amplitude[% map.index %]l0_0()
            amp0_1 = amplitude[% map.index %]l0_1()
            ampct_0 = amplitude[% map.index %]yukct_0(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            heli_amp = square(amp0_0, ampct_1) &
            & + square(amp0_1, ampct_0)
         case (7)
            ! sigma(dim6 X dim6)  with loopcounting
            amp0_1 = amplitude[% map.index %]l0_1()
            ampct_1 = amplitude[% map.index %]yukct_1(logs,scale2,eps)
            heli_amp = square(amp0_1, ampct_1)
         end select[%
     @else %]
         amp0 = amplitude[% map.index %]l0()
         ampct = amplitude[% map.index %]yukct(logs,scale2,eps)
  ...
  @end @for unique_helicity_mappings%]
   ...
   function     samplitudeyukct_h(vecs, logs, scale2, eps, h) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics, only: init_event
      implicit none
      real(ki), dimension([%num_legs%], 4), intent(in) :: vecs
   ...
   @for helicities%]
     ...
     if(debug_nlo_diagrams) then
      ...
      @else %][% 'if not generate_lo_diagrams' %]
         ...
         !---#] reinitialize kinematics:[%
      @if enable_truncation_orders %]
         print*, "In [% process_name asprefix=\_ %]matrix: samplitudel1_h"
         print*, "Truncation order feature not yet supported for"
         print*, "loop induced processes in GoSam3. Terminating."
         stop
         select case (EFTcount)
...
@end @if generate_lo_diagrams%][%
      ...
      case (1)
         ...
         if(corrections_are_qcd) then
            ...
            if (renorm_yukawa) then
               if (present(h)) then
                  amp(3) = amp(3) + samplitudeyukct_h_qp(vecs, renorm_logs, scale2, -1, h)
                  amp(2) = amp(2) + samplitudeyukct_h_qp(vecs, renorm_logs, scale2, 0, h)
               else
                  amp(3) = amp(3) + samplitudeyukct_qp(vecs, renorm_logs, scale2, -1)
                  amp(2) = amp(2) + samplitudeyukct_qp(vecs, renorm_logs, scale2, 0)
               end if
            end if
            [% @end @if generate_yuk_counterterms %]
         ...
         if (renorm_eftwilson) then
            if (present(h)) then
               ampct((/4,3,2/)) = samplitudeeftct_h_qp(vecs, renorm_logs, scale2, h)
            else
               ampct((/4,3,2/)) = samplitudeeftct_qp(vecs, renorm_logs, scale2)
            end if
            ! account for nlo_prefactors in EFT counterterms
...
[% @if generate_eft_counterterms %][%
@if eval topolopy.count.ct .gt. 0 %]
   !---#[ function samplitudeeftct_qp :
   function     samplitudeeftct_qp(vecs, logs, scale2) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics_qp, only: init_event
      implicit none
      real(ki_qp), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @end @for unique_helicity_mappings%]
   ...
   @end @if generate_lo_diagrams %]
   end function samplitudeeftct_qp
   !---#] function samplitudeeftct_qp :
   !---#[ function samplitudeeftct_h_qp :
   function     samplitudeeftct_h_qp(vecs, logs, scale2, h) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics_qp, only: init_event
      implicit none
      real(ki_qp), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @end @for helicities %]
   ...
   @end @if generate_lo_diagrams %]
   end function samplitudeeftct_h_qp
   !---#] function samplitudeeftct_h_qp :
[% @end @if %]
[% @end @if generate_eft_counterterms %]
[% @if generate_yuk_counterterms %]
   !---#[ function samplitudeyukct_qp :
   function     samplitudeyukct_qp(vecs,logs,scale2,eps) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics_qp, only: init_event
      implicit none
      real(ki_qp), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @for unique_helicity_mappings %]
     ...
     @if enable_truncation_orders %]
         ...
         case (0)
            ...
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            amp0_2 = amplitude[% map.index %]l0_2_qp()
            ampct_0 = amplitude[% map.index %]yukct_0_qp(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_0, ampct_0 + ampct_1 + ampct_2) &
            & + square_qp(amp0_1 + amp0_2, ampct_0)
         case (1)
            ! sigma(SM + dim6 X SM + dim6) without loopcounting
            amp0_0 = amplitude[% map.index %]l0_0_qp()
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ...
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_0 + amp0_1 + amp0_2, ampct_0 + ampct_1 + ampct_2)
         case (2)
            ! sigma(SM X SM) + sigma(SM X dim6) with loopcounting
            amp0_0 = amplitude[% map.index %]l0_0_qp()
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ampct_0 = amplitude[% map.index %]yukct_0_qp(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_0, ampct_0 + ampct_1) &
            & + square_qp(amp0_1, ampct_0)
         case (3)
           ! sigma(SM + dim6 X SM + dim6) with loopcounting
            amp0_0 = amplitude[% map.index %]l0_0_qp()
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ampct_0 = amplitude[% map.index %]yukct_0_qp(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_0 + amp0_1, ampct_0 + ampct_1)
         case (4)
            ! sigma(SM X dim6) without loopcounting
            amp0_0 = amplitude[% map.index %]l0_0_qp()
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ...
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_0, ampct_1 + ampct_2) &
            & + square_qp(amp0_1 + amp0_2, ampct_0)
         case (5)
            ! sigma(dim6 X dim6)  without loopcounting
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ...
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            ampct_2 = amplitude[% map.index %]yukct_2_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_1 + amp0_2, ampct_1 + ampct_2)
         case (6)
            ! sigma(SM X dim6) with loopcounting
            amp0_0 = amplitude[% map.index %]l0_0_qp()
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ampct_0 = amplitude[% map.index %]yukct_0_qp(logs,scale2,eps)
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_0, ampct_1) &
            & + square_qp(amp0_1, ampct_0)
         case (7)
            ! sigma(dim6 X dim6)  with loopcounting
            amp0_1 = amplitude[% map.index %]l0_1_qp()
            ampct_1 = amplitude[% map.index %]yukct_1_qp(logs,scale2,eps)
            heli_amp = square_qp(amp0_1, ampct_1)
         end select[%
     @else %]
         amp0 = amplitude[% map.index %]l0()
         ampct = amplitude[% map.index %]yukct(logs,scale2,eps)
  ...
  @end @for unique_helicity_mappings%]
   ...
   !---#] function samplitudeyukct_qp :
   !---#[ function samplitudeyukct_h_qp :
   function     samplitudeyukct_h_qp(vecs, logs, scale2, eps, h) result(amp)
      use [% @if internal OLP_MODE %][% @else %][% process_name%]_[% @end @if %]config, only: logfile
      use [% process_name asprefix=\_ %]kinematics_qp, only: init_event
      implicit none
      real(ki_qp), dimension([%num_legs%], 4), intent(in) :: vecs
  ...
  @end @for helicities %]
   ...
   @end @if generate_lo_diagrams %]
   end function samplitudeyukct_h_qp
   !---#] function samplitudeyukct_h_qp :
[% @end @if generate_yuk_counterterms %]
   ...
   @for unique_helicity_mappings %][%
     ...
     @end @for current_helicities %][%
      ...
      @for current_helicities %][%
      @if enable_truncation_orders %]
         print*, "In [% process_name asprefix=\_ %]matrix: samplitudel1_qp"
         print*, "Truncation order feature not yet supported for"
         print*, "loop induced processes in GoSam3. Terminating."
         stop
         select case (EFTcount)
   ...
   @for helicities%]
     ...
     if(debug_nlo_diagrams) then
      ...
      @else %][% 'if not generate_lo_diagrams' %]
         ...
         !---#] reinitialize kinematics:[%
      @if enable_truncation_orders %]
         print*, "In [% process_name asprefix=\_ %]matrix: samplitudel1_h_qp"
         print*, "Truncation order feature not yet supported for"
         print*, "loop induced processes in GoSam3. Terminating."
         stop
         select case (EFTcount)
