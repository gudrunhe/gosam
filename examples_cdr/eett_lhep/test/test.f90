...
subroutine     compute_gosam_result(vecs, scale2, amp)
   ...
   ! Reference result in dred. No external massless
   ! coloured particles -> no convert_to_cdr shift,
   ! BUT: amplitude unrenormalised, so dred <> cdr
   ! shifts contained in counterterms not included
   ! in GoSam result -> account for that here

   ! comes from dred <> cdr difference in mqwf CT for T and Tbar
   amp(1) = amp(1) + 4d0/3d0*amp(0)
