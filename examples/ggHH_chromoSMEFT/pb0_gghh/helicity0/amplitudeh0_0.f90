...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      ...
      use pb0_gghh_color, only: CF, CA
      use pb0_gghh_kinematics, only: &
      & num_light_quarks, num_gluons
      use pb0_gghh_diagramsh0l0_0, only: amplitudel0 => amplitude
      implicit none
   ...
   function     samplitude(scale2,ok,rational2,opt_amp0,opt_perm)
      use pb0_gghh_config, only: include_eps_terms, include_eps2_terms, &
      & logfile, debug_nlo_diagrams
      use pb0_gghh_globalsl1, only: amp0,perm, use_perm, epspow
      use pb0_gghh_globalsh0, &
     & only: init_lo, rat2
      use pb0_gghh_abbrevd13_0h0, only: init_abbrevd13 => init_abbrev
      use pb0_gghh_abbrevd11_0h0, only: init_abbrevd11 => init_abbrev
      use pb0_gghh_abbrevd2_0h0, only: init_abbrevd2 => init_abbrev
      use pb0_gghh_abbrevd7_0h0, only: init_abbrevd7 => init_abbrev
      use pb0_gghh_abbrevd9_0h0, only: init_abbrevd9 => init_abbrev
      use pb0_gghh_diagramsh0l0_0, only: amplitudel0 => amplitude
      use pb0_gghh_groups
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use pb0_gghh_ninja_0h0, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   use pb0_gghh_config, only: &
      & logfile, debug_nlo_diagrams
   use pb0_gghh_globalsl1, only: epspow
   use pb0_gghh_ninja_0h0, only: ninja_reduce => ninja_reduce_group1
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   real(ki), dimension(-2:0), intent(out) :: samplitude
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   use pb0_gghh_config, only: &
      & logfile, debug_nlo_diagrams
   use pb0_gghh_globalsl1, only: epspow
   use pb0_gghh_ninja_0h0, only: ninja_reduce => ninja_reduce_group2
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   real(ki), dimension(-2:0), intent(out) :: samplitude
...
!---#] subroutine evaluate_group2:
end module pb0_gghh_amplitudeh0_0
