program test
   use pb0_gghh_config, only: ki, logfile, nlo_prefactors
   use pb0_gghh_kinematics, only: dotproduct, boost_to_cms
   use pb0_gghh_model, only: parse
   use pb0_gghh_matrix, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use pb0_gghh_color, only: numcs, CA
   use pb0_gghh_rambo, only: ramb
...
subroutine  print_parameters(scale2)
   use pb0_gghh_config, only: renormalisation, &
