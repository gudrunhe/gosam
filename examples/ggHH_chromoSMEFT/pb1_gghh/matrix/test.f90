program test
   use pb1_gghh_config, only: ki, logfile, nlo_prefactors
   use pb1_gghh_kinematics, only: dotproduct, boost_to_cms
   use pb1_gghh_model, only: parse
   use pb1_gghh_matrix, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use pb1_gghh_color, only: numcs, CA
   use pb1_gghh_rambo, only: ramb
...
subroutine  print_parameters(scale2)
   use pb1_gghh_config, only: renormalisation, &
