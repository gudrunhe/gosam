...
!---#[ function finite_renormalisation:
   ...
   function     samplitude(scale2,ok,rational2,the_col0,opt_perm)
      use pb1_gghh_config, only: include_eps_terms, include_eps2_terms, &
      & logfile, debug_nlo_diagrams
      use pb1_gghh_globalsl1, only: col0,perm, use_perm, epspow
      use pb1_gghh_globalsh0, &
     & only: init_lo, rat2
      use pb1_gghh_abbrevd13_1h0, only: init_abbrevd13 => init_abbrev
      use pb1_gghh_abbrevd11_1h0, only: init_abbrevd11 => init_abbrev
      use pb1_gghh_abbrevd2_1h0, only: init_abbrevd2 => init_abbrev
      use pb1_gghh_abbrevd7_1h0, only: init_abbrevd7 => init_abbrev
      use pb1_gghh_abbrevd9_1h0, only: init_abbrevd9 => init_abbrev
      use pb1_gghh_groups
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use pb1_gghh_ninja_1h0, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   use pb1_gghh_config, only: &
      & logfile, debug_nlo_diagrams
   use pb1_gghh_globalsl1, only: epspow
   use pb1_gghh_ninja_1h0, only: ninja_reduce => ninja_reduce_group1
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   use pb1_gghh_config, only: &
      & logfile, debug_nlo_diagrams
   use pb1_gghh_globalsl1, only: epspow
   use pb1_gghh_ninja_1h0, only: ninja_reduce => ninja_reduce_group2
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
!---#] subroutine evaluate_group2:
end module pb1_gghh_amplitudeh0_1
