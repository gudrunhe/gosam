program test
   use pc17_gghh_config, only: ki, logfile, nlo_prefactors
   use pc17_gghh_kinematics, only: dotproduct, boost_to_cms
   use pc17_gghh_model, only: parse
   use pc17_gghh_matrix, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use pc17_gghh_color, only: numcs, CA
   use pc17_gghh_rambo, only: ramb
...
subroutine  print_parameters(scale2)
   use pc17_gghh_config, only: renormalisation, &
