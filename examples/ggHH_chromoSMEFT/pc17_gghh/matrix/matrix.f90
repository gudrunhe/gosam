module     pc17_gghh_matrix
   ...
   use pc17_gghh_model, only: mdlEFTcount, Nf, NC, sqrt2, init_functions
   use pc17_gghh_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use pc17_gghh_diagramsh0l0_0, only: amplitude0l0_0 => amplitude
   use pc17_gghh_diagramsh0l0_1, only: amplitude0l0_1 => amplitude
   use pc17_gghh_diagramsh0l0_2, only: amplitude0l0_2 => amplitude
   use pc17_gghh_amplitudeh0_0, only: samplitudeh0l1_0 => samplitude, &
     &   finite_renormalisation0_0 => finite_renormalisation
   use pc17_gghh_amplitudeh0_1, only: samplitudeh0l1_1 => samplitude, &
     &   finite_renormalisation0_1 => finite_renormalisation
   use pc17_gghh_amplitudeh0_2, only: samplitudeh0l1_2 => samplitude, &
     &   finite_renormalisation0_2 => finite_renormalisation
   use pc17_gghh_dipoles, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     exitgolem(is_last)
      use pc17_gghh_groups, only: tear_down_golem95
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use pc17_gghh_config, only: logfile
      use pc17_gghh_kinematics, only: init_event
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
      ...
      if (eval_heli(0)) then
         ...
         case (0)
            ...
            heli_amp = square(color_vector_0, color_vector_2)
         case (1)
            ! sigma(SM + dim6 X CHROMO)
            ! color_vector_0 = amplitude0l0_0()
            color_vector_1 = amplitude0l0_1()
            color_vector_2 = amplitude0l0_2()
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      ...
      if (eval_heli(0)) then
         ...
         case(0)
            ...
            heli_amp = samplitudeh0l1_2(real(scale2,ki),my_ok,rational2,amp0_0) &
            &        + samplitudeh0l1_0(real(scale2,ki),my_ok,rational2,amp0_2)
         case(1)
            ! sigma(SM + dim6 X CHROMO)
            ! amp0_0 = amplitude0l0_0()
            amp0_1 = amplitude0l0_1()
            amp0_2 = amplitude0l0_2()
            heli_amp = samplitudeh0l1_2(real(scale2,ki),my_ok,rational2,amp0_1) &
            &        + samplitudeh0l1_1(real(scale2,ki),my_ok,rational2,amp0_2)
         case(2)
            !!! TESTING PURPOSE !!!
            ! sigma(SM + dim6 X SM + dim6)
            ! amp0_0 = amplitude0l0_0()
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use pc17_gghh_color, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use pc17_gghh_color, only: cmat => T1T2
