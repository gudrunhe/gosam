module     pc16_gghh_matrix
   ...
   use pc16_gghh_model, only: mdlEFTcount, Nf, NC, sqrt2, init_functions
   use pc16_gghh_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use pc16_gghh_amplitudeh0_0, only: samplitudeh0l1_0 => samplitude, &
     &   finite_renormalisation0_0 => finite_renormalisation
   use pc16_gghh_amplitudeh0_1, only: samplitudeh0l1_1 => samplitude, &
     &   finite_renormalisation0_1 => finite_renormalisation
   use pc16_gghh_amplitudeh0_2, only: samplitudeh0l1_2 => samplitude, &
     &   finite_renormalisation0_2 => finite_renormalisation
   use pc16_gghh_dipoles, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     exitgolem(is_last)
      use pc16_gghh_groups, only: tear_down_golem95
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use pc16_gghh_config, only: logfile
      use pc16_gghh_kinematics, only: init_event
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      ...
      if (eval_heli(0)) then
         ...
         case(0)
            ...
            heli_amp( 0) = square(colorvec_0(:, 0), colorvec_2(:, 0))
            heli_amp(-1) = square(colorvec_0(:,-1), colorvec_2(:, 0)) &
                       & + square(colorvec_0(:, 0), colorvec_2(:,-1))
            heli_amp(-2) = square(colorvec_0(:,-2), colorvec_2(:, 0)) &
                       & + square(colorvec_0(:,-1), colorvec_2(:,-1)) &
                       & + square(colorvec_0(:, 0), colorvec_2(:,-2))
         case(1)
            ...
            heli_amp(-1) = square(colorvec_1(:,-1), colorvec_2(:, 0)) &
                       & + square(colorvec_1(:, 0), colorvec_2(:,-1))
            heli_amp(-2) = square(colorvec_1(:,-2), colorvec_2(:, 0)) &
                       & + square(colorvec_1(:,-1), colorvec_2(:,-1)) &
                       & + square(colorvec_1(:, 0), colorvec_2(:,-2))
         case(2)
            !!! TESTING PURPOSE !!!
            ...
            heli_amp(-1) = square(colorvec_1(:,-1), colorvec_1(:, 0))
            heli_amp(-2) = square(colorvec_1(:,-2), colorvec_1(:, 0)) &
                       & + square(colorvec_1(:,-1))
         case(3)
            !!! TESTING PURPOSE !!!
            ...
            heli_amp(-2) = square(colorvec_2(:,-2), colorvec_2(:, 0)) &
                       & + square(colorvec_2(:,-1))
         end select
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use pc16_gghh_color, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use pc16_gghh_color, only: cmat => T1T2
