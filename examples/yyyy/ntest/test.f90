program test
   ...
   ! (1) ++-+
   ! (2) ++--
   ! This means, for golem (in-out notation) we have
   ! (0) ++--
   ! (1) +++-
   ! (2) ++++
   ...
   double precision, dimension(0:num_helis-1) :: golem_amps
   ...
   do ievt = 1, NEVT
      ...
      do ihel = 0, num_helis - 1
         ...
         do ic = 1, 2
            ...
            diff = rel_diff(golem_amps(ihel), reference_amps(ihel)**2)
            if (diff .gt. eps) then
               write(unit=ch,fmt="(A3,1x,A30)") "==>", &
                  & "Comparison failed!"
               success = .false.
            end if
            write(unit=ch,fmt="(A18,1x,E23.15)") "GOSAM:", golem_amps(ihel)
            write(unit=ch,fmt="(A18,1x,E23.15)") "GAUGE:", gauge_amps(ihel)
            write(unit=ch,fmt="(A18,1x,E23.15)") "REFERENCE:", &
               & reference_amps(ihel)**2

            diff = rel_diff(golem_amps(ihel), gauge_amps(ihel))
            if (diff .gt. eps) then
