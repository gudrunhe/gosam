...
subroutine     compute_golem_result(vecs, scale2, amp)
   ...
   do ich=1,2
      ...
      write(ch(ich),*) "GOSAM     AMP(1)/AMP(0):", amp(1)/amp(0)
      write(ch(ich),*) "GOSAM     AMP(2)/AMP(0):", amp(2)/amp(0)
      write(ch(ich),*) "GOSAM     AMP(3)/AMP(0):", amp(3)/amp(0)
   end do
...
subroutine     compute_reference_result(vecs, scale2, amp)
   ...
   amp(1) = (-78.713051902606352_ki - 7.0_ki/6.0_ki) * amp(0)
   amp(2:3) = real(irp(2:3), dbl)

   do ich=1,2
      write(ch(ich),*) "REFERENCE AMP(0):       ", amp(0)
      write(ch(ich),*) "REFERENCE AMP(1)/AMP(0):", amp(1)/amp(0)
      write(ch(ich),*) "REFERENCE AMP(2)/AMP(0):", amp(2)/amp(0)
      write(ch(ich),*) "REFERENCE AMP(3)/AMP(0):", amp(3)/amp(0)
   end do
