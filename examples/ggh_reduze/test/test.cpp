...
int main()
{
    ...
    invariants.factoutscale = parameters.mH*parameters.mH;
...
//    integrator.m = 128;
    ...
    // Factor 1/256 obtained from:
    // 1/8 * 1/8 (average over incoming gluon colors)
    // 1/2 * 1/2 (average over incoming gluon spins)

    integral_complex_t loop1_eps0 = result.at(0).at(0).at(0).value;
    integral_real_t me2_loop1_eps0 = std::real(loop1_eps0*std::conj(loop1_eps0));
    std::cout << "GOSAM-XLOOP AMP(0): " << me2_loop1_eps0/256. << std::endl;
    std::cout << "GOSAM       AMP(0): 6.171643144305729e+01" << std::endl;
    std::cout << std::endl;
