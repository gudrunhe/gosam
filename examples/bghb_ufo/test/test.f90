...
subroutine     setup_parameters()
   ...
   use bghb_model, only: init_functions, print_parameter, &
        & mdlMZ, mdlGf, mdlaEWM1, mdlymb, mdlMH, mdlaS
   implicit none
   ...
   ! parameters matching the bghb example using the built-in model
   mdlMZ=295.7246947505437_ki
   mdlGf=1.0_ki/sqrt(2.0_ki)/(246.2185_ki)**2
   mdlaEWM1=12.56637061435919_ki
   mdlymb = 2.937956_ki
   mdlMH = 120.0_ki
   call init_functions()
...
subroutine     compute_reference_result(vecs, scale2, amp)
   ...
   use bghb_model, only: mdlMW, mdlsw, mdlMH, mdlymb
   implicit none
   ...
   t = -2.0d0 * dotproduct(vecs(1,:), vecs(3,:)) + mdlMH**2
   u = mdlMH**2 - s - t

   vev = 0.5d0 * mdlsw * mdlmW
   yB = mdlymb / vev
   
   amp(0) = - 0.25_ki * yB**2 * (mdlMH**4 + u**2) / s / t / 24.0d0
